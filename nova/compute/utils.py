...
NON_INHERITABLE_IMAGE_PROPERTIES = frozenset(['cinder_encryption_key_id', 'cinder_encryption_key_deletion_policy', 'img_signature', 'img_signature_hash_method', 'img_signature_key_type', 'img_signature_certificate_uuid'])
NON_INHERITABLE_IMAGE_NAMESPACES = frozenset(['os_glance'])

@p.trace('exception_to_dict')
...
@p.trace('_get_fault_details')
def _get_fault_details(exc_info, error_code):
    details = ''
    if exc_info and error_code == 500:
        details = ''.join(traceback.format_exception(exc_info[0], exc_info[1], exc_info[2]))
    return str(details)

@p.trace('add_instance_fault_from_exc')
...
@p.trace('get_device_name_for_instance')
...
@p.trace('default_device_names_for_instance')
...
@p.trace('check_max_disk_devices_to_attach')
...
@p.trace('is_volume_backed_instance')
...
@p.trace('get_value_from_system_metadata')
...
@p.trace('notify_usage_exists')
def notify_usage_exists(notifier, context, instance_ref, host, current_period=False, ignore_missing_network_data=True, system_metadata=None, extra_usage_info=None):
    ...
    extra_info = dict(audit_period_beginning=str(audit_start), audit_period_ending=str(audit_end), bandwidth={}, image_meta=image_meta)
    if extra_usage_info:
        extra_info.update(extra_usage_info)
    notify_about_instance_usage(notifier, context, instance_ref, 'exists', extra_usage_info=extra_info)
    audit_period = instance_notification.AuditPeriodPayload(audit_period_beginning=audit_start, audit_period_ending=audit_end)
    payload = instance_notification.InstanceExistsPayload(context=context, instance=instance_ref, audit_period=audit_period, bandwidth=[])
...
@p.trace('notify_about_instance_usage')
def notify_about_instance_usage(notifier, context, instance, event_suffix, network_info=None, extra_usage_info=None, fault=None):
    ...
    if not extra_usage_info:
        extra_usage_info = {}
    usage_info = notifications.info_from_instance(context, instance, network_info, populate_image_ref_url=True, **extra_usage_info)
    if fault:
...
@p.trace('_get_fault_and_priority_from_exception')
...
@p.trace('notify_about_instance_action')
@rpc.if_notifications_enabled
def notify_about_instance_action(context, instance, host, action, phase=None, source=fields.NotificationSource.COMPUTE, exception=None, bdms=None):
    ...
    payload = instance_notification.InstanceActionPayload(context=context, instance=instance, fault=fault, bdms=bdms)
    notification = instance_notification.InstanceActionNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=source), event_type=notification_base.EventType(object='instance', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_instance_create')
@rpc.if_notifications_enabled
def notify_about_instance_create(context, instance, host, phase=None, exception=None, bdms=None):
    ...
    """
    (fault, priority) = _get_fault_and_priority_from_exception(exception)
    payload = instance_notification.InstanceCreatePayload(context=context, instance=instance, fault=fault, bdms=bdms)
    notification = instance_notification.InstanceCreateNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=fields.NotificationAction.CREATE, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_scheduler_action')
@rpc.if_notifications_enabled
def notify_about_scheduler_action(context, request_spec, action, phase=None, source=fields.NotificationSource.SCHEDULER):
    ...
    :param source: the source of the notification
    """
    payload = reqspec_notification.RequestSpecPayload(request_spec=request_spec)
    notification = scheduler_notification.SelectDestinationsNotification(context=context, priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=CONF.host, source=source), event_type=notification_base.EventType(object='scheduler', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_volume_attach_detach')
@rpc.if_notifications_enabled
def notify_about_volume_attach_detach(context, instance, host, action, phase, volume_id=None, exception=None):
    """Send versioned notification about the action made on the instance
    :param instance: the instance which the action performed on
    :param host: the host emitting the notification
    :param action: the name of the action
    ...
    """
    (fault, priority) = _get_fault_and_priority_from_exception(exception)
    payload = instance_notification.InstanceActionVolumePayload(context=context, instance=instance, fault=fault, volume_id=volume_id)
    notification = instance_notification.InstanceActionVolumeNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_instance_rescue_action')
@rpc.if_notifications_enabled
def notify_about_instance_rescue_action(context, instance, host, rescue_image_ref, phase=None, exception=None):
    """Send versioned notification about the action made on the instance
    ...
    """
    (fault, priority) = _get_fault_and_priority_from_exception(exception)
    payload = instance_notification.InstanceActionRescuePayload(context=context, instance=instance, fault=fault, rescue_image_ref=rescue_image_ref)
    notification = instance_notification.InstanceActionRescueNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=fields.NotificationAction.RESCUE, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_keypair_action')
@rpc.if_notifications_enabled
def notify_about_keypair_action(context, keypair, action, phase):
    ...
    notification = keypair_notification.KeypairNotification(priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=CONF.host, source=fields.NotificationSource.API), event_type=notification_base.EventType(object='keypair', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_volume_swap')
@rpc.if_notifications_enabled
def notify_about_volume_swap(context, instance, host, phase, old_volume_id, new_volume_id, exception=None):
    ...
    """
    (fault, priority) = _get_fault_and_priority_from_exception(exception)
    payload = instance_notification.InstanceActionVolumeSwapPayload(context=context, instance=instance, fault=fault, old_volume_id=old_volume_id, new_volume_id=new_volume_id)
    instance_notification.InstanceActionVolumeSwapNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=fields.NotificationAction.VOLUME_SWAP, phase=phase), payload=payload).emit(context)

@p.trace('notify_about_instance_snapshot')
@rpc.if_notifications_enabled
def notify_about_instance_snapshot(context, instance, host, phase, snapshot_image_id):
    ...
    """
    payload = instance_notification.InstanceActionSnapshotPayload(context=context, instance=instance, fault=None, snapshot_image_id=snapshot_image_id)
    instance_notification.InstanceActionSnapshotNotification(context=context, priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=fields.NotificationAction.SNAPSHOT, phase=phase), payload=payload).emit(context)

@p.trace('notify_about_resize_prep_instance')
@rpc.if_notifications_enabled
def notify_about_resize_prep_instance(context, instance, host, phase, new_flavor):
    ...
    """
    payload = instance_notification.InstanceActionResizePrepPayload(context=context, instance=instance, fault=None, new_flavor=flavor_notification.FlavorPayload(flavor=new_flavor))
    instance_notification.InstanceActionResizePrepNotification(context=context, priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='instance', action=fields.NotificationAction.RESIZE_PREP, phase=phase), payload=payload).emit(context)

@p.trace('notify_about_server_group_update')
...
@p.trace('notify_about_aggregate_update')
...
@p.trace('notify_about_aggregate_action')
@rpc.if_notifications_enabled
def notify_about_aggregate_action(context, aggregate, action, phase):
    ...
    notification = aggregate_notification.AggregateNotification(priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=CONF.host, source=fields.NotificationSource.API), event_type=notification_base.EventType(object='aggregate', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_aggregate_cache')
@rpc.if_notifications_enabled
def notify_about_aggregate_cache(context, aggregate, host, image_status, index, total):
    ...
    payload = aggregate_notification.AggregateCachePayload(aggregate, host, index, total)
...
@p.trace('notify_about_server_group_action')
@rpc.if_notifications_enabled
def notify_about_server_group_action(context, group, action):
    payload = sg_notification.ServerGroupPayload(group)
    notification = sg_notification.ServerGroupNotification(priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=CONF.host, source=fields.NotificationSource.API), event_type=notification_base.EventType(object='server_group', action=action), payload=payload)
    notification.emit(context)

@p.trace('notify_about_server_group_add_member')
@rpc.if_notifications_enabled
def notify_about_server_group_add_member(context, group_id):
    group = objects.InstanceGroup.get_by_uuid(context, group_id)
    payload = sg_notification.ServerGroupPayload(group)
    notification = sg_notification.ServerGroupNotification(priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=CONF.host, source=fields.NotificationSource.API), event_type=notification_base.EventType(object='server_group', action=fields.NotificationAction.ADD_MEMBER), payload=payload)
    notification.emit(context)

@p.trace('notify_about_instance_rebuild')
@rpc.if_notifications_enabled
def notify_about_instance_rebuild(context, instance, host, action=fields.NotificationAction.REBUILD, phase=None, source=fields.NotificationSource.COMPUTE, exception=None, bdms=None):
    ...
    """
    (fault, priority) = _get_fault_and_priority_from_exception(exception)
    payload = instance_notification.InstanceActionRebuildPayload(context=context, instance=instance, fault=fault, bdms=bdms)
    notification = instance_notification.InstanceActionRebuildNotification(context=context, priority=priority, publisher=notification_base.NotificationPublisher(host=host, source=source), event_type=notification_base.EventType(object='instance', action=action, phase=phase), payload=payload)
    notification.emit(context)

@p.trace('notify_about_metrics_update')
@rpc.if_notifications_enabled
def notify_about_metrics_update(context, host, host_ip, nodename, monitor_metric_list):
    ...
    """
    payload = metrics_notification.MetricsPayload(host=host, host_ip=host_ip, nodename=nodename, monitor_metric_list=monitor_metric_list)
    notification = metrics_notification.MetricsNotification(context=context, priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='metrics', action=fields.NotificationAction.UPDATE), payload=payload)
    notification.emit(context)

@p.trace('notify_about_libvirt_connect_error')
@rpc.if_notifications_enabled
...
def notify_about_volume_usage(context, vol_usage, host):
    ...
    notification = volume_notification.VolumeUsageNotification(context=context, priority=fields.NotificationPriority.INFO, publisher=notification_base.NotificationPublisher(host=host, source=fields.NotificationSource.COMPUTE), event_type=notification_base.EventType(object='volume', action=fields.NotificationAction.USAGE), payload=payload)
    notification.emit(context)

@p.trace('notify_about_compute_task_error')
@rpc.if_notifications_enabled
def notify_about_compute_task_error(context, action, instance_uuid, request_spec, state, exception):
    ...
    """
    if request_spec is not None and (not isinstance(request_spec, objects.RequestSpec)):
        request_spec = objects.RequestSpec.from_primitives(context, request_spec, {})
    (fault, _) = _get_fault_and_priority_from_exception(exception)
    payload = task_notification.ComputeTaskPayload(instance_uuid=instance_uuid, request_spec=request_spec, state=state, reason=fault)
    notification = task_notification.ComputeTaskNotification(priority=fields.NotificationPriority.ERROR, publisher=notification_base.NotificationPublisher(host=CONF.host, source=fields.NotificationSource.CONDUCTOR), event_type=notification_base.EventType(object='compute_task', action=action, phase=fields.NotificationPhase.ERROR), payload=payload)
    notification.emit(context)

@p.trace('refresh_info_cache_for_instance')
...
@p.trace('check_num_instances_quota')
def check_num_instances_quota(context, instance_type, min_count, max_count, project_id=None, user_id=None, orig_num_req=None):
    ...
    try:
        ...
        if allowed <= 0:
            ...
            return check_num_instances_quota(context, instance_type, min_count, allowed, project_id=project_id, user_id=user_id)
        else:
...
@p.trace('remove_shelved_keys_from_system_metadata')
...
@p.trace('create_image')
def create_image(context, instance, name, image_type, image_api, extra_properties=None):
    ...
    image_meta = initialize_instance_snapshot_metadata(context, instance, name, properties)
    if image_type == 'snapshot':
        image_meta.pop('disk_format', None)
        image_meta.pop('container_format', None)
    return image_api.create(context, image_meta)

@p.trace('initialize_instance_snapshot_metadata')
def initialize_instance_snapshot_metadata(context, instance, name, extra_properties=None):
    ...
    keys_to_pop = set(CONF.non_inheritable_image_properties).union(NON_INHERITABLE_IMAGE_PROPERTIES)
    for ns in NON_INHERITABLE_IMAGE_NAMESPACES:
        keys_to_pop |= {key for key in properties if key.startswith(ns)}
    for key in keys_to_pop:
        properties.pop(key, None)
    properties.update(extra_properties)
    return image_meta

@p.trace('delete_image')
def delete_image(context, instance, image_api, image_id, log_exc_info=False):
    ...
    """
    LOG.debug('Cleaning up image %s', image_id, instance=instance, log_exc_info=log_exc_info)
    try:
        image_api.delete(context, image_id)
    except exception.ImageNotFound:
        pass
    except Exception:
        LOG.exception('Error while trying to clean up image %s', image_id, instance=instance)

@p.trace('may_have_ports_or_volumes')
...
@p.trace('get_stashed_volume_connector')
def get_stashed_volume_connector(bdm, instance):
    ...
    if 'connection_info' in bdm and bdm.connection_info is not None:
        ...
        if connector:
            ...
            if instance.host is None and may_have_ports_or_volumes(instance):
...
class EventReporter(object):
    ...
    def __init__(self, context, event_name, host, *instance_uuids, graceful_exit=False):
    ...
    def __enter__(self):
        for uuid in self.instance_uuids:
            objects.InstanceActionEvent.event_start(self.context, uuid, self.event_name, want_result=False, host=self.host)
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        for uuid in self.instance_uuids:
            try:
                ...
                with excutils.save_and_reraise_exception(reraise=not self.graceful_exit):
                    if self.graceful_exit:
                        return True
        return False

@p.trace('wrap_instance_event')
def wrap_instance_event(prefix, graceful_exit=False):
    ...
    def helper(function):
        ...
        def decorated_function(self, context, *args, **kwargs):
            ...
            with EventReporter(context, event_name, host, instance_uuid, graceful_exit=graceful_exit):
                return function(self, context, *args, **kwargs)
        return decorated_function
    return helper

@p.trace_cls('UnlimitedSemaphore')
...
@p.trace('notify_about_instance_delete')
@contextlib.contextmanager
def notify_about_instance_delete(notifier, context, instance, delete_type='delete', source=fields.NotificationSource.API):
    try:
        notify_about_instance_usage(notifier, context, instance, '%s.start' % delete_type)
        if delete_type in ['delete', 'soft_delete']:
            notify_about_instance_action(context, instance, host=CONF.host, source=source, action=delete_type, phase=fields.NotificationPhase.START)
        yield
    finally:
        notify_about_instance_usage(notifier, context, instance, '%s.end' % delete_type)
        if delete_type in ['delete', 'soft_delete']:
...
@p.trace('update_pci_request_spec_with_allocated_interface_name')
def update_pci_request_spec_with_allocated_interface_name(context, report_client, pci_requests, provider_mapping):
    ...
    def needs_update(pci_request, mapping):
        return pci_request.requester_id and pci_request.requester_id in mapping
    for pci_request in pci_requests:
        if needs_update(pci_request, provider_mapping):
            ...
            if len(provider_uuids) != 1:
                raise exception.AmbiguousResourceProviderForPCIRequest(providers=provider_uuids, requester=pci_request.requester_id)
            dev_rp_name = report_client.get_resource_provider_name(context, provider_uuids[0])
...
def delete_arqs_if_needed(context, instance):
    ...
    LOG.debug('Calling Cyborg to delete ARQs for instance %(instance)s', {'instance': instance.uuid})
    try:
        cyclient.delete_arqs_for_instance(instance.uuid)
    except exception.AcceleratorRequestOpFailed as e:
        LOG.exception('Failed to delete accelerator requests for instance %s. Exception: %s', instance.uuid, e)