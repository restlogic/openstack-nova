...
def lvcreate(size, lv, vg, preallocated=None):
    ...
    if not preallocated:
        ...
        cmd.extend(['-L', '%db' % preallocated, '--virtualsize', '%db' % size])
    cmd.extend(['-n', lv, vg])
...
@nova.privsep.sys_admin_pctxt.entrypoint
def vginfo(vg):
    return processutils.execute('vgs', '--noheadings', '--nosuffix', '--separator', '|', '--units', 'b', '-o', 'vg_size,vg_free', vg, attempts=3, delay_on_retry=True)

@p.trace('lvlist')
@nova.privsep.sys_admin_pctxt.entrypoint
def lvlist(vg):
    return processutils.execute('lvs', '--noheadings', '-o', 'lv_name', vg, attempts=3, delay_on_retry=True)

@p.trace('lvinfo')
@nova.privsep.sys_admin_pctxt.entrypoint
def lvinfo(path):
    return processutils.execute('lvs', '-o', 'vg_all,lv_all', '--separator', '|', path)
...
@p.trace('unprivileged_resize2fs')
...
@p.trace('create_partition_table')
@nova.privsep.sys_admin_pctxt.entrypoint
def create_partition_table(device, style, check_exit_code=True):
    processutils.execute('parted', '--script', device, 'mklabel', style, check_exit_code=check_exit_code)

@p.trace('create_partition')
@nova.privsep.sys_admin_pctxt.entrypoint
def create_partition(device, style, start, end, check_exit_code=True):
    processutils.execute('parted', '--script', device, '--', 'mkpart', style, start, end, check_exit_code=check_exit_code)

@p.trace('list_partitions')
@nova.privsep.sys_admin_pctxt.entrypoint
def list_partitions(device):
    return unprivileged_list_partitions(device)

@p.trace('unprivileged_list_partitions')
def unprivileged_list_partitions(device):
    """Return partition information (num, size, type) for a device."""
    (out, _err) = processutils.execute('parted', '--script', '--machine', device, 'unit s', 'print')
...
@p.trace('resize_partition')
@nova.privsep.sys_admin_pctxt.entrypoint
def resize_partition(device, start, end, bootable):
    processutils.execute('parted', '--script', device, 'rm', '1')
    processutils.execute('parted', '--script', device, 'mkpart', 'primary', '%ds' % start, '%ds' % end)
    if bootable:
        processutils.execute('parted', '--script', device, 'set', '1', 'boot', 'on')

@p.trace('ext_journal_disable')
@nova.privsep.sys_admin_pctxt.entrypoint
def ext_journal_disable(device):
    processutils.execute('tune2fs', '-O ^has_journal', device)

@p.trace('ext_journal_enable')
@nova.privsep.sys_admin_pctxt.entrypoint
...
@p.trace('get_file_extension_for_os_type')
def get_file_extension_for_os_type(os_type, default_ephemeral_format, specified_fs=None):
    global _MKFS_COMMAND
    global _DEFAULT_MKFS_COMMAND
...
@p.trace('_inner_configurable_mkfs')
@nova.privsep.sys_admin_pctxt.entrypoint
...
@p.trace('configurable_mkfs')
def configurable_mkfs(os_type, fs_label, target, run_as_root, default_ephemeral_format, specified_fs=None):
    global _MKFS_COMMAND
    global _DEFAULT_MKFS_COMMAND
    mkfs_command = (_MKFS_COMMAND.get(os_type, _DEFAULT_MKFS_COMMAND) or '') % {'fs_label': fs_label, 'target': target}
    if mkfs_command:
