...
def validate_name(context: nova_context.RequestContext, name: str):
    ...
    try:
        return neutronv20.find_resourceid_by_name_or_id(neutron, 'security_group', name, context.project_id)
...
@p.trace('new_group_ingress_rule')
def new_group_ingress_rule(grantee_group_id, protocol, from_port, to_port):
    return _new_ingress_rule(protocol, from_port, to_port, group_id=grantee_group_id)

@p.trace('new_cidr_ingress_rule')
def new_cidr_ingress_rule(grantee_cidr, protocol, from_port, to_port):
    return _new_ingress_rule(protocol, from_port, to_port, cidr=grantee_cidr)

@p.trace('_new_ingress_rule')
def _new_ingress_rule(ip_protocol, from_port, to_port, group_id=None, cidr=None):
    ...
    if group_id:
        ...
        if ip_proto_upper == 'ICMP' and from_port is None and (to_port is None):
            from_port = -1
            to_port = -1
        elif ip_proto_upper in ['TCP', 'UDP'] and from_port is None and (to_port is None):
            from_port = 1
            to_port = 65535
    elif cidr:
        values['cidr'] = cidr
    if ip_protocol and from_port is not None and (to_port is not None):
        ...
        try:
            ...
            if ip_protocol.upper() == 'ICMP':
                raise exception.InvalidInput(reason=_('Type and Code must be integers for ICMP protocol type'))
            else:
        ...
        if ip_protocol.upper() in ['TCP', 'UDP'] and from_port > to_port:
            raise exception.InvalidPortRange(from_port=from_port, to_port=to_port, msg='Former value cannot be greater than the later')
        if ip_protocol.upper() in ['TCP', 'UDP'] and (from_port < 1 or to_port > 65535):
            raise exception.InvalidPortRange(from_port=from_port, to_port=to_port, msg='Valid %s ports should be between 1-65535' % ip_protocol.upper())
        if ip_protocol.upper() == 'ICMP' and (from_port < -1 or from_port > 255 or to_port < -1 or (to_port > 255)):
            raise exception.InvalidPortRange(from_port=from_port, to_port=to_port, msg='For ICMP, the type:code must be valid')
...
@p.trace('create_security_group_rule')
def create_security_group_rule(context, security_group, new_rule):
    if _rule_exists(security_group, new_rule):
        msg = _('This rule already exists in group %s') % new_rule['parent_group_id']
        raise exception.Invalid(msg)
    return add_rules(context, new_rule['parent_group_id'], security_group['name'], [new_rule])[0]

@p.trace('_rule_exists')
...
@p.trace('populate_security_groups')
...
@p.trace('_convert_to_nova_security_group_format')
def _convert_to_nova_security_group_format(security_group):
    ...
    for rule in security_group.get('security_group_rules', []):
        if rule['direction'] == 'ingress':
            nova_group['rules'].append(_convert_to_nova_security_group_rule_format(rule))
    return nova_group

@p.trace('_convert_to_nova_security_group_rule_format')
...
def list(context, project, search_opts=None):
    ...
    for security_group in security_groups:
        converted_rules.append(_convert_to_nova_security_group_format(security_group))
...
def add_rules(context, id, name, vals):
    ...
    try:
        ...
        if e.status_code == 404:
            ...
            LOG.exception('Neutron Error adding rules to security group %s', name)
            raise exception.SecurityGroupLimitExceeded(str(e))
    ...
    converted_rules = []
    for rule in rules:
        converted_rules.append(_convert_to_nova_security_group_rule_format(rule))
    return converted_rules

@p.trace('_make_neutron_security_group_dict')
def _make_neutron_security_group_dict(name, description):
    return {'security_group': {'name': name, 'description': description}}

@p.trace('_make_neutron_security_group_rules_list')
...
@p.trace('_get_ports_from_server_list')
...
@p.trace('_get_secgroups_from_port_list')
...
@p.trace('get_instances_security_groups_bindings')
def get_instances_security_groups_bindings(context, servers, detailed=False):
    ...
    security_groups = _get_secgroups_from_port_list(ports, neutron, fields=fields)
    ...
    for port in ports:
        for port_sg_id in port.get('security_groups', []):
            ...
            if port_sg:
                if detailed:
                    sg_entry = _convert_to_nova_security_group_format(port_sg)
...
@p.trace('get_instance_security_groups')
def get_instance_security_groups(context, instance, detailed=False):
    ...
    sg_bindings = get_instances_security_groups_bindings(context, servers, detailed)
    return sg_bindings.get(instance.uuid, [])

@p.trace('_has_security_group_requirements')
...
def add_to_instance(context, instance, security_group_name):
    ...
    for port in ports:
        if not _has_security_group_requirements(port):
            LOG.warning('Cannot add security group %(name)s to %(instance)s since the port %(port_id)s does not meet security requirements', {'name': security_group_name, 'instance': instance.uuid, 'port_id': port['id']})
        ...
        try:
            LOG.info('Adding security group %(security_group_id)s to port %(port_id)s', {'security_group_id': security_group_id, 'port_id': port['id']})
...
def remove_from_instance(context, instance, security_group_name):
    ...
    for port in ports:
        ...
        updated_port = {'security_groups': port['security_groups']}
        try:
            LOG.info('Removing security group %(security_group_id)s from port %(port_id)s', {'security_group_id': security_group_id, 'port_id': port['id']})
            neutron.update_port(port['id'], {'port': updated_port})
