...
def _load_auth_plugin(conf):
    ...
    if conf.neutron.auth_type is None:
        LOG.error('The [neutron] section of your nova configuration file must be configured for authentication with the networking service endpoint. See the networking service install guide for details: https://docs.openstack.org/neutron/latest/install/')
    err_msg = _('Unknown auth type: %s') % conf.neutron.auth_type
    raise neutron_client_exc.Unauthorized(message=err_msg)

@p.trace('get_binding_profile')
...
@p.trace('update_instance_cache_with_nw_info')
    ...
    def proxy(self, obj):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                LOG.error('Neutron client was not able to generate a valid admin token, please verify Neutron admin credential located in nova.conf')
...
def get_client(context, admin=False):
    ...
    client_args = dict(session=session, auth=auth_plugin, global_request_id=context.global_id, connect_retries=CONF.neutron.http_retries)
    adap = utils.get_ksa_adapter('network', ksa_auth=auth_plugin, ksa_session=session)
    client_args = dict(client_args, service_type=adap.service_type, service_name=adap.service_name, interface=adap.interface, region_name=adap.region_name, endpoint_override=adap.endpoint_override)
    return ClientWrapper(clientv20.Client(**client_args), admin=admin or context.is_admin)

@p.trace('_get_ksa_client')
def _get_ksa_client(context, admin=False):
    ...
    auth_plugin = _get_auth_plugin(context, admin=admin)
    session = _get_session()
    client = utils.get_ksa_adapter('network', ksa_auth=auth_plugin, ksa_session=session)
    client.additional_headers = {'accept': 'application/json'}
    client.connect_retries = CONF.neutron.http_retries
    return client

@p.trace('_is_not_duplicate')
def _is_not_duplicate(item, items, items_list_name, instance):
    present = item in items
    if present:
        LOG.warning('%(item)s already exists in list: %(list_name)s containing: %(items)s. ignoring it', {'item': item, 'list_name': items_list_name, 'items': items}, instance=instance)
    return not present

@p.trace('_ensure_no_port_binding_failure')
...
class API(base.Base):
    ...
    def _update_port_with_migration_profile(self, instance, port_id, port_profile, admin_client):
        try:
            updated_port = admin_client.update_port(port_id, {'port': {constants.BINDING_PROFILE: port_profile}})
            return updated_port
        except Exception as ex:
            with excutils.save_and_reraise_exception():
                LOG.error('Unable to update binding profile for port: %(port)s due to failure: %(error)s', {'port': port_id, 'error': ex}, instance=instance)
    ...
    def setup_networks_on_host(self, context, instance, host=None, teardown=False):
        ...
        if port_migrating or teardown:
            search_opts = {'device_id': instance.uuid, 'tenant_id': instance.project_id, constants.BINDING_HOST_ID: instance.host}
    ...
    def _delete_port_bindings(self, context, ports, host):
        ...
        if failed_port_ids:
            msg = _('Failed to delete binding for port(s) %(port_ids)s and host %(host)s.') % {'port_ids': ','.join(failed_port_ids), 'host': host}
    ...
    def _cleanup_created_port(self, port_client, port_id, instance):
        try:
            ...
            LOG.exception('Failed to delete port %(port_id)s while cleaning up after an error.', {'port_id': port_id}, instance=instance)
    ...
    def _create_port_minimal(self, port_client, instance, network_id, fixed_ip=None, security_group_ids=None):
        ...
        try:
            ...
            if port.get(constants.RESOURCE_REQUEST, None):
                msg = 'The auto-created port %(port_id)s is being deleted due to its network having QoS policy.'
                LOG.info(msg, {'port_id': port_id})
                self._cleanup_created_port(port_client, port_id, instance)
                raise exception.NetworksWithQoSPolicyNotSupported(instance_uuid=instance.uuid, network_id=network_id)
            try:
            ...
            LOG.warning('Neutron error: %(ip)s is not a valid IP address for network %(network_id)s.', {'ip': fixed_ip, 'network_id': network_id}, instance=instance)
            msg = _('Fixed IP %(ip)s is not a valid ip address for network %(network_id)s.') % {'ip': fixed_ip, 'network_id': network_id}
            raise exception.InvalidInput(reason=msg)
        except (neutron_client_exc.IpAddressInUseClient, neutron_client_exc.IpAddressAlreadyAllocatedClient):
            LOG.warning('Neutron error: Fixed IP %s is already in use.', fixed_ip, instance=instance)
            ...
            LOG.warning('Neutron error: Port quota exceeded in tenant: %s', port_req_body['port']['tenant_id'], instance=instance)
            raise exception.PortLimitExceeded()
        except neutron_client_exc.IpAddressGenerationFailureClient:
            LOG.warning('Neutron error: No more fixed IPs in network: %s', network_id, instance=instance)
            raise exception.NoMoreFixedIps(net=network_id)
        except neutron_client_exc.NeutronClientException:
            with excutils.save_and_reraise_exception():
                LOG.exception('Neutron error creating port on network %s', network_id, instance=instance)
    ...
    def _update_port(self, port_client, instance, port_id, port_req_body):
        try:
            ...
            LOG.warning('Neutron error: MAC address %(mac)s is already in use on network %(network)s.', {'mac': mac_address, 'network': network_id}, instance=instance)
            raise exception.PortInUse(port_id=mac_address)
        except neutron_client_exc.HostNotCompatibleWithFixedIpsClient:
            network_id = port_req_body['port'].get('network_id')
            LOG.warning('Neutron error: Tried to bind a port with fixed_ips to a host in the wrong segment on network %(network)s.', {'network': network_id}, instance=instance)
    ...
    def _unbind_ports(self, context, ports, neutron, port_client=None):
        ...
        for port_id in ports:
            ...
            try:
                port = self._show_port(context, port_id, neutron_client=neutron, fields=[constants.BINDING_PROFILE, 'network_id'])
            except exception.PortNotFound:
                LOG.debug('Unable to show port %s as it no longer exists.', port_id)
                return
            except Exception:
                LOG.exception("Unable to get binding:profile for port '%s'", port_id)
                port_profile = {}
                network = {}
            else:
                ...
                if net_id in networks:
                    ...
                    network = neutron.show_network(net_id, fields=['dns_domain']).get('network')
                    networks[net_id] = network
            for profile_key in ('pci_vendor_info', 'pci_slot', constants.ALLOCATION):
    ...
    def _process_security_groups(self, instance, neutron, security_groups):
        ...
        if len(security_groups):
            ...
            user_security_groups = neutron.list_security_groups(fields=sg_fields, **search_opts).get('security_groups')
    ...
    def allocate_for_instance(self, context, instance, requested_networks, security_groups=None, bind_host_id=None, resource_provider_mapping=None):
        ...
        neutron = get_client(context)
        admin_client = get_client(context, admin=True)
        (requested_ports_dict, ordered_networks) = self._validate_requested_port_ids(context, instance, admin_client, requested_networks)
        ...
        for port in requested_ports_dict.values():
            if port.get(constants.RESOURCE_REQUEST):
                ...
                profile[constants.ALLOCATION] = resource_provider_mapping[port['id']][0]
        ...
        (ordered_nets, ordered_port_ids, preexisting_port_ids, created_port_ids) = self._update_ports_for_instance(context, instance, neutron, admin_client, requests_and_created_ports, nets, bind_host_id, requested_ports_dict)
        nw_info = self.get_instance_nw_info(context, instance, networks=ordered_nets, port_ids=ordered_port_ids, admin_client=admin_client, preexisting_port_ids=preexisting_port_ids)
        return network_model.NetworkInfo([vif for vif in nw_info if vif['id'] in created_port_ids + preexisting_port_ids])

    def _update_ports_for_instance(self, context, instance, neutron, admin_client, requests_and_created_ports, nets, bind_host_id, requested_ports_dict):
        ...
        for (request, created_port_id) in requests_and_created_ports:
            ...
            if requested_ports_dict and request.port_id in requested_ports_dict and get_binding_profile(requested_ports_dict[request.port_id]):
                port_req_body['port'][constants.BINDING_PROFILE] = get_binding_profile(requested_ports_dict[request.port_id])
            try:
    ...
    def bind_ports_to_host(self, context, instance, host, vnic_types=None, port_profiles=None):
        ...
        for vif in network_info:
            ...
            if resp:
                ...
                LOG.error('Binding failed for port %s and host %s. Error: (%s %s)', port_id, host, resp.status_code, resp.text, instance=instance)
                for rollback_port_id in bindings_by_port_id:
                    try:
                        self.delete_port_binding(context, rollback_port_id, host)
                    except exception.PortBindingDeletionFailed:
                        LOG.warning('Failed to remove binding for port %s on host %s.', rollback_port_id, host, instance=instance)
    ...
    def _create_port_binding(context, client, port_id, data):
        ...
        """
        return client.post('/v2.0/ports/%s/bindings' % port_id, json=data, raise_exc=False, global_request_id=context.global_id)
    ...
    def delete_port_binding(self, context, port_id, host):
        ...
        if resp:
            LOG.debug('Deleted binding for port %s and host %s.', port_id, host)
        elif resp.status_code != 404:
            LOG.error('Unexpected error trying to delete binding for port %s and host %s. Code: %s. Error: %s', port_id, host, resp.status_code, resp.text)
            raise exception.PortBindingDeletionFailed(port_id=port_id, host=host)
    ...
    def _delete_port_binding(context, client, port_id, host):
        ...
        :return: requests.Response object
        """
        return client.delete('/v2.0/ports/%s/bindings/%s' % (port_id, host), raise_exc=False, global_request_id=context.global_id)
    ...
    def activate_port_binding(self, context, port_id, host):
        ...
        if resp:
            LOG.debug('Activated binding for port %s and host %s.', port_id, host)
        elif resp.status_code == 409:
            LOG.warning('Binding for port %s and host %s is already active.', port_id, host)
        else:
            LOG.error('Unexpected error trying to activate binding for port %s and host %s. Code: %s. Error: %s', port_id, host, resp.status_code, resp.text)
            raise exception.PortBindingActivationFailed(port_id=port_id, host=host)
    ...
    def _activate_port_binding(context, client, port_id, host):
        ...
        :return: requests.Response object
        """
        return client.put('/v2.0/ports/%s/bindings/%s/activate' % (port_id, host), raise_exc=False, global_request_id=context.global_id)
    ...
    def _get_port_binding(context, client, port_id, host):
        ...
        :return: requests.Response object
        """
        return client.get('/v2.0/ports/%s/bindings/%s' % (port_id, host), raise_exc=False, global_request_id=context.global_id)
    ...
    def _populate_neutron_binding_profile(self, instance, pci_request_id, port_req_body):
        ...
        """
        if pci_request_id:
            pci_devices = pci_manager.get_instance_pci_devs(instance, pci_request_id)
            if not pci_devices:
                LOG.error('Unable to find PCI device using PCI request ID in list of claimed instance PCI devices: %s. Is the [pci]/passthrough_whitelist configuration correct?', list(instance.pci_devices), instance=instance)
                raise exception.PciDeviceNotFound(_('PCI device not found for request ID %s.') % pci_request_id)
    ...
    def _populate_pci_mac_address(instance, pci_request_id, port_req_body):
        ...
        if pci_request_id is not None:
            ...
            if len(pci_devs) != 1:
                LOG.error('PCI request %s does not have a unique device associated with it. Unable to determine MAC address', pci_request_id, instance=instance)
                return
            pci_dev = pci_devs[0]
            if pci_dev.dev_type == obj_fields.PciDeviceType.SRIOV_PF:
                try:
                    ...
                    LOG.error('Could not determine MAC address for %(addr)s, error: %(e)s', {'addr': pci_dev.address, 'e': e}, instance=instance)
                else:
    ...
    def _update_port_dns_name(self, context, instance, network, port_id, neutron):
        ...
        if self._has_dns_extension() and network.get('dns_domain'):
            try:
                ...
                LOG.warning('Neutron error: Instance hostname %(hostname)s is not a valid DNS name', {'hostname': instance.hostname}, instance=instance)
        ...
        for port in ports:
            try:
                ...
                if e.status_code == 404:
                    LOG.warning('Port %s does not exist', port, instance=instance)
                else:
                    exceptions.append(e)
                    LOG.warning('Failed to delete port %s for instance.', port, instance=instance, exc_info=True)
    ...
    def deallocate_for_instance(self, context, instance, **kwargs):
        ...
        update_instance_cache_with_nw_info(self, context, instance, network_model.NetworkInfo([]))
    ...
    def deallocate_port_for_instance(self, context, instance, port_id):
        ...
        try:
            ...
            LOG.debug('Unable to determine port %s resource allocation information as the port no longer exists.', port_id)
            port = None
        ...
        if port:
            ...
            if resource_request and allocated_rp:
                port_allocation = {allocated_rp: {'resources': resource_request.get('resources', {})}}
        else:
            for vif in instance.get_network_info():
                if vif['id'] == port_id:
                    ...
                    if rp_uuid:
                        LOG.warning('Port %s disappeared during deallocate but it had resource allocation on resource provider %s. Resource allocation for this port may be leaked.', port_id, rp_uuid, instance=instance)
                    break
    ...
    def get_instance_nw_info(self, context, instance, **kwargs):
        ...
        with lockutils.lock('refresh_cache-%s' % instance.uuid):
            ...
            update_instance_cache_with_nw_info(self, context, instance, nw_info=result)
        return result

    def _get_instance_nw_info(self, context, instance, networks=None, port_ids=None, admin_client=None, preexisting_port_ids=None, refresh_vif_id=None, force_refresh=False, **kwargs):
        if force_refresh:
            LOG.debug('Forcefully refreshing network info cache for instance', instance=instance)
        elif refresh_vif_id:
            LOG.debug('Refreshing network info cache for port %s', refresh_vif_id, instance=instance)
        else:
            LOG.debug('Building network info cache for instance', instance=instance)
        compute_utils.refresh_info_cache_for_instance(context, instance)
        nw_info = self._build_network_info_model(context, instance, networks, port_ids, admin_client, preexisting_port_ids, refresh_vif_id, force_refresh=force_refresh)
    ...
    def remove_fixed_ip_from_instance(self, context, instance, address):
        ...
        for p in ports:
            ...
            return self._get_instance_nw_info(context, instance)
        raise exception.FixedIpNotFoundForInstance(instance_uuid=instance.uuid, ip=address)
    ...
    def _get_physnet_tunneled_info(self, context, neutron, net_id):
        ...
        if self._has_multi_provider_extension(context, neutron=neutron):
            ...
            if segments:
                ...
                raise exception.NovaException(message=msg)
        net = neutron.show_network(net_id, fields=['provider:physical_network', 'provider:network_type']).get('network')
        return (net.get('provider:physical_network'), net.get('provider:network_type') in constants.L3_NETWORK_TYPES)
    ...
    def _get_port_vnic_info(self, context, neutron, port_id):
        ...
        """
        fields = ['binding:vnic_type', constants.BINDING_PROFILE, 'network_id', constants.RESOURCE_REQUEST, constants.NUMA_POLICY]
        port = self._show_port(context, port_id, neutron_client=neutron, fields=fields)
    ...
    def create_resource_requests(self, context, requested_networks, pci_requests=None, affinity_policy=None):
        ...
        for request_net in requested_networks:
            ...
            if request_net.port_id:
                ...
                (physnet, tunneled_) = self._get_physnet_tunneled_info(context, neutron, network_id)
                if resource_request:
                    resource_requests.append(objects.RequestGroup.from_port_request(context=None, port_uuid=request_net.port_id, port_resource_request=resource_request))
            ...
            if vnic_type in network_model.VNIC_TYPES_SRIOV:
                ...
                request = objects.InstancePCIRequest(count=1, spec=[spec], request_id=uuidutils.generate_uuid(), requester_id=requester_id)
        ...
        return (objects.NetworkMetadata(physnets=physnets, tunneled=tunneled), resource_requests)
    ...
    def _can_auto_allocate_network(self, context, neutron):
        ...
        try:
            ...
            LOG.debug('Unable to auto-allocate networks. %s', str(ex))
            return False
    ...
    def _auto_allocate_network(self, instance, neutron):
        ...
        try:
            ...
            LOG.error('Automatically allocated network %(network_id)s was not found.', {'network_id': topology['id']}, instance=instance)
            raise exception.UnableToAutoAllocateNetwork(project_id=project_id)
    ...
    def get_requested_resource_for_instance(self, context, instance_uuid):
        ...
        """
        neutron = get_client(context, admin=True)
        data = neutron.list_ports(device_id=instance_uuid, fields=['id', 'resource_request'])
        resource_requests = []
        for port in data.get('ports', []):
            if port.get('resource_request'):
                resource_requests.append(objects.RequestGroup.from_port_request(context=None, port_uuid=port['id'], port_resource_request=port['resource_request']))
    ...
    def associate_floating_ip(self, context, instance, floating_address, fixed_address, affect_auto_assigned=False):
        ...
        if fip['port_id']:
            try:
                self._update_inst_info_cache_for_disassociated_fip(context, instance, client, fip)
            except Exception as e:
                LOG.warning('An error occurred while trying to refresh the network info cache for an instance associated with port %s. Error: %s', fip['port_id'], e)

    def _update_inst_info_cache_for_disassociated_fip(self, context, instance, client, fip):
        ...
        msg_dict = dict(address=fip['floating_ip_address'], instance_id=orig_instance_uuid)
        LOG.info('re-assign floating IP %(address)s from instance %(instance_id)s', msg_dict, instance=instance)
        orig_instance = self._get_instance_by_uuid_using_api_db(context, orig_instance_uuid)
        if orig_instance:
            update_instance_cache_with_nw_info(self, orig_instance._context, orig_instance)
        else:
            LOG.info('Unable to refresh the network info cache for instance %s after disassociating floating IP %s. If the instance still exists, its info cache may be healed automatically.', orig_instance_uuid, fip['id'])
    ...
    def get_floating_ip(self, context, id):
        ...
        try:
            fip['network_details'] = client.show_network(network_uuid)['network']
        except neutron_client_exc.NetworkNotFoundClient:
            raise exception.NetworkNotFound(network_id=network_uuid)
        if not self._has_fip_port_details_extension(context, client):
            port_id = fip['port_id']
            try:
                fip['port_details'] = client.show_port(port_id)['port']
            except neutron_client_exc.PortNotFoundClient:
    ...
    def get_floating_ips_by_project(self, context):
        ...
        if not fips:
            return fips
        networks = {net['id']: net for net in self._get_available_networks(context, project_id, [fip['floating_network_id'] for fip in fips], client)}
        for fip in fips:
            network_uuid = fip['floating_network_id']
        ...
        if not self._has_fip_port_details_extension(context, client):
            ports = {port['id']: port for port in client.list_ports(**{'tenant_id': project_id})['ports']}
            for fip in fips:
                port_id = fip['port_id']
                if port_id in ports:
    ...
    def get_vifs_by_instance(self, context, instance):
        return objects.VirtualInterfaceList.get_by_instance_uuid(context, instance.uuid)
    ...
    def _safe_get_floating_ips(self, client, **kwargs):
        ...
        try:
            ...
            if e.status_code == 404:
                return []
            with excutils.save_and_reraise_exception():
                LOG.exception('Unable to access floating IP for %s', ', '.join(['%s %s' % (k, v) for (k, v) in kwargs.items()]))
    ...
    def disassociate_and_release_floating_ip(self, context, instance, floating_ip):
        ...
        def _release_floating_ip_and_refresh_cache(self, context, instance, floating_ip):
            self._release_floating_ip(context, floating_ip['floating_ip_address'], raise_if_associated=False)
        if instance:
    ...
    def migrate_instance_start(self, context, instance, migration):
        ...
        if not self.supports_port_binding_extension(context):
            LOG.debug('Neutron is not new enough to perform early destination host port binding activation. Port bindings will be updated later.', instance=instance)
            return
        client = _get_ksa_client(context, admin=True)
        dest_host = migration['dest_compute']
        for vif in instance.get_network_info():
            resp = self._get_port_binding(context, client, vif['id'], dest_host)
            if resp:
                if resp.json()['binding']['status'] != 'ACTIVE':
                    self.activate_port_binding(context, vif['id'], dest_host)
                else:
                    LOG.debug('Port %s binding to destination host %s is already ACTIVE.', vif['id'], dest_host, instance=instance)
            elif resp.status_code == 404:
                return
            else:
                LOG.error('Unexpected error trying to get binding info for port %s and destination host %s. Code: %i. Error: %s', vif['id'], dest_host, resp.status_code, resp.text)

    def migrate_instance_finish(self, context, instance, migration, provider_mappings):
        ...
        :param provider_mappings: a dict of list of resource provider uuids
            keyed by port uuid
        """
        self._update_port_binding_for_instance(context, instance, migration.dest_compute, migration=migration, provider_mappings=provider_mappings)
    ...
    def _nw_info_build_network(self, context, port, networks, subnets):
        ...
        for net in networks:
            ...
            LOG.warning('Network %(id)s not matched with the tenants network! The ports tenant %(tenant_id)s will be used.', {'id': port['network_id'], 'tenant_id': tenant_id})
        bridge = None
        ...
        (physnet, tunneled) = self._get_physnet_tunneled_info(context, neutron, port['network_id'])
        network = network_model.Network(id=port['network_id'], bridge=bridge, injected=CONF.flat_injected, label=network_name, tenant_id=tenant_id, mtu=network_mtu, physical_network=physnet, tunneled=tunneled)
    ...
    def _build_vif_model(self, context, client, current_neutron_port, networks, preexisting_port_ids):
        ...
        (network, ovs_interfaceid) = self._nw_info_build_network(context, current_neutron_port, networks, subnets)
    ...
    def _build_network_info_model(self, context, instance, networks=None, port_ids=None, admin_client=None, preexisting_port_ids=None, refresh_vif_id=None, force_refresh=False):
        ...
        if refresh_vif_id is not None:
            ...
            if nw_info:
                current_neutron_port = current_neutron_port_map.get(refresh_vif_id)
                if current_neutron_port:
                    networks = self._get_available_networks(context, instance.project_id, [current_neutron_port['network_id']], client)
                    refreshed_vif = self._build_vif_model(context, client, current_neutron_port, networks, preexisting_port_ids)
                    for (index, vif) in enumerate(nw_info):
                        if vif['id'] == refresh_vif_id:
                            nw_info[index] = refreshed_vif
                            LOG.debug('Updated VIF entry in instance network info cache for port %s.', refresh_vif_id, instance=instance)
                            break
                    else:
                        nw_info.append(refreshed_vif)
                        LOG.debug('Added VIF to instance network info cache for port %s.', refresh_vif_id, instance=instance)
                else:
                    for (index, vif) in enumerate(nw_info):
                        if vif['id'] == refresh_vif_id:
                            LOG.info('Port %s from network info_cache is no longer associated with instance in Neutron. Removing from network info_cache.', refresh_vif_id, instance=instance)
                            del nw_info[index]
                            break
                return nw_info
        nw_info_refresh = networks is None and port_ids is None
        if nw_info_refresh and force_refresh:
            port_ids = self._get_ordered_port_list(context, instance, current_neutron_ports)
            net_ids = [current_neutron_port_map.get(port_id).get('network_id') for port_id in port_ids]
            networks = self._get_available_networks(context, instance.project_id, net_ids, client)
        else:
        ...
        for port_id in port_ids:
            ...
            if current_neutron_port:
                vif = self._build_vif_model(context, client, current_neutron_port, networks, preexisting_port_ids)
                nw_info.append(vif)
            elif nw_info_refresh:
                LOG.info('Port %s from network info_cache is no longer associated with instance in Neutron. Removing from network info_cache.', port_id, instance=instance)
        return nw_info
    ...
    def _get_ordered_port_list(self, context, instance, current_neutron_ports):
        ...
        port_order_list = sorted(port_uuid_to_index_map, key=lambda k: port_uuid_to_index_map[k])
    ...
    def setup_instance_network_on_host(self, context, instance, host, migration=None, provider_mappings=None):
        ...
        :param provider_mappings: a dict of lists of resource provider uuids
            keyed by port uuid
        """
        self._update_port_binding_for_instance(context, instance, host, migration, provider_mappings)
    ...
    def cleanup_instance_network_on_host(self, context, instance, host):
        ...
        """
        if not self.supports_port_binding_extension(context):
            LOG.info("Neutron extension '%s' is not supported; not cleaning up port bindings for host %s.", constants.PORT_BINDING_EXTENDED, host, instance=instance)
            return
        search_opts = {'device_id': instance.uuid, 'tenant_id': instance.project_id, 'fields': ['id']}
        data = self.list_ports(context, **search_opts)
    ...
    def _get_pci_mapping_for_migration(self, instance, migration):
        if not instance.migration_context:
            return {}
        revert = migration and migration.get('status') == 'reverted'
    ...
    def _get_port_pci_slot(self, context, instance, port):
        ...
        for r in instance.pci_requests.requests:
            ...
            LOG.debug('No PCI request found for port %s', port['id'], instance=instance)
            return None
        for d in instance.pci_devices:
            ...
            LOG.debug('No PCI device found for request %s', request.request_id, instance=instance)
            return None
        return device.address

    def _update_port_binding_for_instance(self, context, instance, host, migration=None, provider_mappings=None):
        neutron = get_client(context, admin=True)
        search_opts = {'device_id': instance.uuid, 'tenant_id': instance.project_id}
        data = neutron.list_ports(**search_opts)
        port_updates = []
        ports = data['ports']
        FAILED_VIF_TYPES = (network_model.VIF_TYPE_UNBOUND, network_model.VIF_TYPE_BINDING_FAILED)
        for p in ports:
            updates = {}
            binding_profile = get_binding_profile(p)
            vif_type = p.get('binding:vif_type')
            if p.get(constants.BINDING_HOST_ID) != host or vif_type in FAILED_VIF_TYPES:
                updates[constants.BINDING_HOST_ID] = host
                updates['device_owner'] = 'compute:%s' % instance.availability_zone
            ...
            if vnic_type in network_model.VNIC_TYPES_SRIOV:
                if migration is not None:
                    if not migration.is_live_migration:
                        pci_mapping = self._get_pci_mapping_for_migration(instance, migration)
                    ...
                    if pci_slot:
                        ...
                        updates[constants.BINDING_PROFILE] = binding_profile
            if p.get('resource_request') and (migration is None or not migration.is_live_migration):
                if not provider_mappings:
                    LOG.warning('Provider mappings are not available to the compute service but are required for ports with a resource request. If compute RPC API versions are pinned for a rolling upgrade, you will need to retry this operation once the RPC version is unpinned and the nova-compute services are all upgraded.', instance=instance)
                    raise exception.PortUpdateFailed(port_id=p['id'], reason=_('Provider mappings are not available to the compute service but are required for ports with a resource request.'))
                binding_profile[constants.ALLOCATION] = provider_mappings[p['id']][0]
                updates[constants.BINDING_PROFILE] = binding_profile
            port_updates.append((p['id'], updates))
        for (port_id, updates) in port_updates:
            if updates:
                LOG.info('Updating port %(port)s with attributes %(attributes)s', {'port': port_id, 'attributes': updates}, instance=instance)
                try:
                    neutron.update_port(port_id, {'port': updates})
                except Exception:
                    with excutils.save_and_reraise_exception():
                        LOG.exception('Unable to update binding details for port %s', port_id, instance=instance)
    ...
    def update_instance_vnic_index(self, context, instance, vif, index):
        ...
        if constants.VNIC_INDEX_EXT in self.extensions:
            ...
            try:
                ...
                with excutils.save_and_reraise_exception():
                    LOG.exception('Unable to update instance VNIC index for port %s.', vif['id'], instance=instance)

    def get_segment_ids_for_network(self, context: nova.context.RequestContext, network_id: str) -> ty.List[str]:
        ...
        client = get_client(context)
        try:
            subnets = client.list_subnets(network_id=network_id, fields='segment_id')['subnets']
        except neutron_client_exc.NeutronClientException as e:
            raise exception.InvalidRoutedNetworkConfiguration('Failed to get segment IDs for network %s' % network_id) from e
        return [subnet['segment_id'] for subnet in subnets if subnet['segment_id'] is not None]

    def get_segment_id_for_subnet(self, context: nova.context.RequestContext, subnet_id: str) -> ty.Optional[str]:
        ...
        try:
            ...
            raise exception.InvalidRoutedNetworkConfiguration('Subnet %s not found' % subnet_id) from e
        return subnet.get('segment_id')

@p.trace('_ensure_requested_network_ordering')
