...
CONF = cfg.CONF
LEGACY_VIFS = {model.VIF_TYPE_DVS, model.VIF_TYPE_IOVISOR, model.VIF_TYPE_802_QBG, model.VIF_TYPE_802_QBH, model.VIF_TYPE_HW_VEB, model.VIF_TYPE_HOSTDEV, model.VIF_TYPE_IB_HOSTDEV, model.VIF_TYPE_MIDONET, model.VIF_TYPE_TAP, model.VIF_TYPE_MACVTAP}

@p.trace('_get_vif_name')
...
@p.trace('_get_hybrid_bridge_name')
...
@p.trace('_set_vhostuser_settings')
...
@p.trace('nova_to_osvif_instance')
def nova_to_osvif_instance(instance):
    ...
    """
    info = objects.instance_info.InstanceInfo(uuid=instance.uuid, name=instance.name)
    if instance.obj_attr_is_set('project_id') and instance.project_id is not None:
        info.project_id = instance.project_id
    return info

@p.trace('_nova_to_osvif_ip')
def _nova_to_osvif_ip(ip):
    ...
    return objects.fixed_ip.FixedIP(address=ip['address'], floating_ips=floating_ips)

@p.trace('_nova_to_osvif_ips')
def _nova_to_osvif_ips(ips):
    ...
    """
    return objects.fixed_ip.FixedIPList(objects=[_nova_to_osvif_ip(ip) for ip in ips])

@p.trace('_nova_to_osvif_route')
def _nova_to_osvif_route(route):
    ...
    """
    obj = objects.route.Route(cidr=route['cidr'])
    if route['interface'] is not None:
        obj.interface = route['interface']
    if route['gateway'] is not None and route['gateway']['address'] is not None:
        obj.gateway = route['gateway']['address']
    return obj

@p.trace('_nova_to_osvif_routes')
...
def _nova_to_osvif_subnet(subnet):
    ...
    obj = objects.subnet.Subnet(dns=dnsaddrs, ips=_nova_to_osvif_ips(subnet['ips']), routes=_nova_to_osvif_routes(subnet['routes']))
    if subnet['cidr'] is not None:
        obj.cidr = subnet['cidr']
    if subnet['gateway'] is not None and subnet['gateway']['address'] is not None:
        obj.gateway = subnet['gateway']['address']
    return obj

@p.trace('_nova_to_osvif_subnets')
...
@p.trace('_nova_to_osvif_network')
...
def _get_vif_instance(vif, cls, plugin, **kwargs):
    ...
    """
    return cls(id=vif['id'], address=vif['address'], network=_nova_to_osvif_network(vif['network']), has_traffic_filtering=vif.is_neutron_filtering_enabled(), preserve_on_delete=vif['preserve_on_delete'], active=vif['active'], plugin=plugin, **kwargs)

@p.trace('_set_representor_datapath_offload_settings')
def _set_representor_datapath_offload_settings(vif, obj):
    ...
    """
    datapath_offload = objects.vif.DatapathOffloadRepresentor(representor_name=_get_vif_name(vif), representor_address=vif['profile']['pci_slot'])
    obj.port_profile.datapath_offload = datapath_offload

@p.trace('_get_vnic_direct_vif_instance')
def _get_vnic_direct_vif_instance(vif, port_profile, plugin, set_bridge=True):
    ...
    """
    obj = _get_vif_instance(vif, objects.vif.VIFHostDevice, port_profile=port_profile, plugin=plugin, dev_address=vif['profile']['pci_slot'], dev_type=objects.fields.VIFHostDeviceDevType.ETHERNET)
    if set_bridge and vif['network']['bridge'] is not None:
        obj.network.bridge = vif['network']['bridge']
    return obj

@p.trace('_get_ovs_representor_port_profile')
def _get_ovs_representor_port_profile(vif):
    ...
    """
    return objects.vif.VIFPortProfileOVSRepresentor(interface_id=vif.get('ovs_interfaceid') or vif['id'], representor_name=_get_vif_name(vif), representor_address=vif['profile']['pci_slot'])

@p.trace('_nova_to_osvif_vif_bridge')
def _nova_to_osvif_vif_bridge(vif):
    obj = _get_vif_instance(vif, objects.vif.VIFBridge, plugin='linux_bridge', vif_name=_get_vif_name(vif))
    if vif['network']['bridge'] is not None:
        obj.bridge_name = vif['network']['bridge']
    return obj

@p.trace('_nova_to_osvif_vif_ovs')
def _nova_to_osvif_vif_ovs(vif):
    vif_name = _get_vif_name(vif)
    vnic_type = vif.get('vnic_type', model.VNIC_TYPE_NORMAL)
    profile = objects.vif.VIFPortProfileOpenVSwitch(interface_id=vif.get('ovs_interfaceid') or vif['id'], datapath_type=vif['details'].get(model.VIF_DETAILS_OVS_DATAPATH_TYPE))
    if vnic_type in (model.VNIC_TYPE_DIRECT, model.VNIC_TYPE_VDPA):
        obj = _get_vnic_direct_vif_instance(vif, port_profile=_get_ovs_representor_port_profile(vif), plugin='ovs')
        _set_representor_datapath_offload_settings(vif, obj)
    elif vif.is_hybrid_plug_enabled():
        obj = _get_vif_instance(vif, objects.vif.VIFBridge, port_profile=profile, plugin='ovs', vif_name=vif_name, bridge_name=_get_hybrid_bridge_name(vif))
    else:
        obj = _get_vif_instance(vif, objects.vif.VIFOpenVSwitch, port_profile=profile, plugin='ovs', vif_name=vif_name)
        if vif['network']['bridge'] is not None:
            obj.bridge_name = vif['network']['bridge']
    return obj

@p.trace('_nova_to_osvif_vif_agilio_ovs')
def _nova_to_osvif_vif_agilio_ovs(vif):
    vnic_type = vif.get('vnic_type', model.VNIC_TYPE_NORMAL)
    if vnic_type == model.VNIC_TYPE_DIRECT:
        obj = _get_vnic_direct_vif_instance(vif, plugin='agilio_ovs', port_profile=_get_ovs_representor_port_profile(vif))
        _set_representor_datapath_offload_settings(vif, obj)
    elif vnic_type == model.VNIC_TYPE_VIRTIO_FORWARDER:
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, port_profile=_get_ovs_representor_port_profile(vif), plugin='agilio_ovs', vif_name=_get_vif_name(vif))
        _set_representor_datapath_offload_settings(vif, obj)
...
@p.trace('_nova_to_osvif_vif_vhostuser')
def _nova_to_osvif_vif_vhostuser(vif):
    if vif['details'].get(model.VIF_DETAILS_VHOSTUSER_FP_PLUG, False):
        if vif['details'].get(model.VIF_DETAILS_VHOSTUSER_OVS_PLUG, False):
            profile = objects.vif.VIFPortProfileFPOpenVSwitch(interface_id=vif.get('ovs_interfaceid') or vif['id'], datapath_type=vif['details'].get(model.VIF_DETAILS_OVS_DATAPATH_TYPE))
            ...
            if vif['network']['bridge'] is not None:
                profile.bridge_name = vif['network']['bridge']
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, plugin='vhostuser_fp', vif_name=_get_vif_name(vif), port_profile=profile)
        _set_vhostuser_settings(vif, obj)
        return obj
    elif vif['details'].get(model.VIF_DETAILS_VHOSTUSER_OVS_PLUG, False):
        profile = objects.vif.VIFPortProfileOpenVSwitch(interface_id=vif.get('ovs_interfaceid') or vif['id'], datapath_type=vif['details'].get(model.VIF_DETAILS_OVS_DATAPATH_TYPE))
        vif_name = ('vhu' + vif['id'])[:model.NIC_NAME_LEN]
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, port_profile=profile, plugin='ovs', vif_name=vif_name)
        if vif['network']['bridge'] is not None:
            obj.bridge_name = vif['network']['bridge']
        _set_vhostuser_settings(vif, obj)
        return obj
    elif vif['details'].get(model.VIF_DETAILS_VHOSTUSER_VROUTER_PLUG, False):
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, plugin='contrail_vrouter', vif_name=_get_vif_name(vif))
        _set_vhostuser_settings(vif, obj)
        return obj
    else:
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, plugin='noop', vif_name=_get_vif_name(vif))
        _set_vhostuser_settings(vif, obj)
        return obj

@p.trace('_nova_to_osvif_vif_ivs')
def _nova_to_osvif_vif_ivs(vif):
    if vif.is_hybrid_plug_enabled():
        obj = _get_vif_instance(vif, objects.vif.VIFBridge, plugin='ivs', vif_name=_get_vif_name(vif), bridge_name=_get_hybrid_bridge_name(vif))
    else:
        obj = _get_vif_instance(vif, objects.vif.VIFGeneric, plugin='ivs', vif_name=_get_vif_name(vif))
    return obj

@p.trace('_nova_to_osvif_vif_vrouter')
def _nova_to_osvif_vif_vrouter(vif):
    vif_name = _get_vif_name(vif)
    vnic_type = vif.get('vnic_type', model.VNIC_TYPE_NORMAL)
    if vnic_type == model.VNIC_TYPE_NORMAL:
        obj = _get_vif_instance(vif, objects.vif.VIFGeneric, plugin='vrouter', vif_name=vif_name)
    elif vnic_type == model.VNIC_TYPE_DIRECT:
        obj = _get_vnic_direct_vif_instance(vif, port_profile=objects.vif.VIFPortProfileBase(), plugin='vrouter', set_bridge=False)
        _set_representor_datapath_offload_settings(vif, obj)
    elif vnic_type == model.VNIC_TYPE_VIRTIO_FORWARDER:
        obj = _get_vif_instance(vif, objects.vif.VIFVHostUser, port_profile=objects.vif.VIFPortProfileBase(), plugin='vrouter', vif_name=vif_name)
        _set_representor_datapath_offload_settings(vif, obj)
        _set_vhostuser_settings(vif, obj)
    else:
