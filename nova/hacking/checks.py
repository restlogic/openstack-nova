...
cfg_opt_re = re.compile('.*[\\s\\[]cfg\\.[a-zA-Z]*Opt\\(')
rule_default_re = re.compile('.*RuleDefault\\(')
policy_enforce_re = re.compile('.*_ENFORCER\\.enforce\\(')
virt_file_re = re.compile('\\./nova/(?:tests/)?virt/(\\w+)/')
virt_import_re = re.compile('^\\s*(?:import|from) nova\\.(?:tests\\.)?virt\\.(\\w+)')
virt_config_re = re.compile("CONF\\.import_opt\\('.*?', 'nova\\.virt\\.(\\w+)('|.)")
asse_trueinst_re = re.compile('(.)*assertTrue\\(isinstance\\((\\w|\\.|\\\'|\\"|\\[|\\])+, (\\w|\\.|\\\'|\\"|\\[|\\])+\\)\\)')
asse_equal_type_re = re.compile('(.)*assertEqual\\(type\\((\\w|\\.|\\\'|\\"|\\[|\\])+\\), (\\w|\\.|\\\'|\\"|\\[|\\])+\\)')
asse_equal_in_end_with_true_or_false_re = re.compile('assertEqual\\((\\w|[][.\'\\"])+ in (\\w|[][.\'\\", ])+, (True|False)\\)')
asse_equal_in_start_with_true_or_false_re = re.compile('assertEqual\\((True|False), (\\w|[][.\'\\"])+ in (\\w|[][.\'\\", ])+\\)')
asse_true_false_with_in_or_not_in = re.compile('assert(True|False)\\((\\w|[][.\'\\"])+( not)? in (\\w|[][.\'\\",])+(, .*)?\\)')
asse_true_false_with_in_or_not_in_spaces = re.compile('assert(True|False)\\((\\w|[][.\'\\"])+( not)? in [\\[|\'|\\"](\\w|[][.\'\\", ])+[\\[|\'|\\"](, .*)?\\)')
asse_raises_regexp = re.compile('assertRaisesRegexp\\(')
conf_attribute_set_re = re.compile('CONF\\.[a-z0-9_.]+\\s*=\\s*\\w')
translated_log = re.compile('(.)*LOG\\.\\w+\\(\\s*_\\(\\s*(\'|\\")')
mutable_default_args = re.compile('^\\s*def .+\\((.+=\\{\\}|.+=\\[\\])')
string_translation = re.compile('[^_]*_\\(\\s*(\'|\\")')
underscore_import_check = re.compile('(.)*import _(.)*')
import_translation_for_log_or_exception = re.compile('(.)*(from\\snova.i18n\\simport)\\s_')
custom_underscore_check = re.compile('(.)*_\\s*=\\s*(.)*')
api_version_re = re.compile('@.*\\bapi_version\\b')
dict_constructor_with_list_copy_re = re.compile('.*\\bdict\\((\\[)?(\\(|\\[)')
decorator_re = re.compile('@.*')
http_not_implemented_re = re.compile('raise .*HTTPNotImplemented\\(')
spawn_re = re.compile('.*(eventlet|greenthread)\\.(?P<spawn_part>spawn(_n)?)\\(.*\\)')
contextlib_nested = re.compile('^with (contextlib\\.)?nested\\(')
doubled_words_re = re.compile('\\b(then?|[iao]n|i[fst]|but|f?or|at|and|[dt]o)\\s+\\1\\b')
log_remove_context = re.compile('(.)*LOG\\.(.*)\\(.*(context=[_a-zA-Z0-9].*)+.*\\)')
return_not_followed_by_space = re.compile('^\\s*return(?:\\(|{|\\"|\'|#).*$')
uuid4_re = re.compile('uuid4\\(\\)($|[^\\.]|\\.hex)')
redundant_import_alias_re = re.compile('import (?:.*\\.)?(.+) as \\1$')
yield_not_followed_by_space = re.compile('^\\s*yield(?:\\(|{|\\[|\\"|\').*$')
asse_regexpmatches = re.compile('(assertRegexpMatches|assertNotRegexpMatches)\\(')
privsep_file_re = re.compile('^nova/privsep[./]')
privsep_import_re = re.compile('^(?:import|from).*\\bprivsep\\b')
disguised_as_tuple_re = re.compile(' in \\(([\'"]?)[a-zA-Z0-9_.]+\\1\\)')
mock_assert_method_re = re.compile('\\.((called_once(_with)*|has_calls)|mock_assert_(called(_(once|with|once_with))?|any_call|has_calls|not_called)|(asser|asset|asssert|assset)_(called(_(once|with|once_with))?|any_call|has_calls|not_called))\\(')
mock_attribute_re = re.compile('[\\.\\(](retrun_value)[,=\\s]')
useless_assertion_re = re.compile('\\.((assertIsNone)\\(None|(assertTrue)\\((True|\\d+|\'.+\'|\\".+\\")),')
mock_assert_has_calls_re = re.compile('\\.assert_has_calls\\s?=')

@p.trace_cls('BaseASTChecker')
...
@p.trace('no_db_session_in_public_api')
@core.flake8ext
...
@p.trace('_get_virt_name')
def _get_virt_name(regex, data):
    m = regex.match(data)
    if m is None:
        return None
    driver = m.group(1)
    if driver in ['test_virt_drivers', 'driver', 'disk', 'api', 'imagecache', 'cpu', 'hardware', 'image']:
        return None
    return driver

@p.trace('import_no_virt_driver_import_deps')
@core.flake8ext
...
@p.trace('import_no_virt_driver_config_deps')
@core.flake8ext
...
@p.trace('no_import_translation_in_tests')
@core.flake8ext
...
@p.trace('no_setting_conf_directly_in_tests')
@core.flake8ext
...
@p.trace('no_mutable_default_args')
@core.flake8ext
...
@p.trace('check_explicit_underscore_import')
@core.flake8ext
...
@p.trace('check_api_version_decorator')
@core.flake8ext
def check_api_version_decorator(logical_line, previous_logical, blank_before, filename):
    msg = 'N332: the api_version decorator must be the first decorator on a method.'
    if blank_before == 0 and re.match(api_version_re, logical_line) and re.match(decorator_re, previous_logical):
        yield (0, msg)

@p.trace_cls('CheckForTransAdd')
...
@p.trace_cls('_FindVariableReferences')
...
@p.trace_cls('CheckForUncalledTestClosure')
class CheckForUncalledTestClosure(BaseASTChecker):
    ...
    def visit_FunctionDef(self, node):
        if not os.path.basename(self._filename).startswith('test_') and os.path.basename(self._filename) != 'stdin':
            return
        closures = []
        references = []
        ...
        if missed:
            self.add_error(node, 'N349: Test closures not called: %s' % ','.join(missed))

@p.trace('assert_true_or_false_with_in')
@core.flake8ext
...
@p.trace('dict_constructor_with_list_copy')
@core.flake8ext
...
def assert_equal_in(logical_line):
    ...
    N338
    """
    res = asse_equal_in_start_with_true_or_false_re.search(logical_line) or asse_equal_in_end_with_true_or_false_re.search(logical_line)
    if res:
        yield (0, 'N338: Use assertIn/NotIn(A, B) rather than assertEqual(A in B, True/False) when checking collection contents.')

@p.trace('check_http_not_implemented')
@core.flake8ext
def check_http_not_implemented(logical_line, filename, noqa):
    msg = 'N339: HTTPNotImplemented response must be implemented with common raise_feature_not_supported().'
    if noqa:
        return
...
@p.trace('check_greenthread_spawns')
@core.flake8ext
...
@p.trace('check_no_contextlib_nested')
@core.flake8ext
def check_no_contextlib_nested(logical_line, filename):
    msg = 'N341: contextlib.nested is deprecated. With Python 2.7 and later the with-statement supports multiple nested objects. See https://docs.python.org/2/library/contextlib.html#contextlib.nested for more information. nova.test.nested() is an alternative as well.'
    if contextlib_nested.match(logical_line):
        yield (0, msg)

@p.trace('check_config_option_in_central_place')
@core.flake8ext
def check_config_option_in_central_place(logical_line, filename):
    msg = "N342: Config options should be in the central location '/nova/conf/*'. Do not declare new config options outside of that folder."
    if 'nova/conf/' in filename:
        return
    conf_exceptions = ['nova/cmd/manage.py', 'nova/cmd/policy.py', 'nova/cmd/status.py', 'nova/tests']
    if any((f in filename for f in conf_exceptions)):
        return
    if cfg_opt_re.match(logical_line):
        yield (0, msg)

@p.trace('check_policy_registration_in_central_place')
@core.flake8ext
def check_policy_registration_in_central_place(logical_line, filename):
    msg = 'N350: Policy registration should be in the central location(s) "/nova/policies/*"'
...
def check_policy_enforce(logical_line, filename):
    ...
    N351
    """
    msg = 'N351: nova.policy._ENFORCER.enforce() should not be used. Use the authorize() method instead.'
    if policy_enforce_re.match(logical_line):
        yield (0, msg)

@p.trace('check_doubled_words')
@core.flake8ext
...
def check_context_log(logical_line, filename, noqa):
    ...
    if log_remove_context.match(logical_line):
        yield (0, "N353: Nova is using oslo.context's RequestContext which means the context object is in scope when doing logging using oslo.log, so no need to pass it as kwarg.")

@p.trace('no_assert_equal_true_false')
@core.flake8ext
def no_assert_equal_true_false(logical_line):
    ...
    N355
    """
    _start_re = re.compile('assert(Not)?Equal\\((True|False),')
    _end_re = re.compile('assert(Not)?Equal\\(.*,\\s+(True|False)\\)$')
    if _start_re.search(logical_line) or _end_re.search(logical_line):
        yield (0, 'N355: assertEqual(A, True|False), assertEqual(True|False, A), assertNotEqual(A, True|False), or assertEqual(True|False, A) sentences must not be used. Use assertTrue(A) or assertFalse(A) instead')

@p.trace('no_assert_true_false_is_not')
@core.flake8ext
def no_assert_true_false_is_not(logical_line):
    ...
    N356
    """
    _re = re.compile('assert(True|False)\\(.+\\s+is\\s+(not\\s+)?.+\\)$')
    if _re.search(logical_line):
        yield (0, 'N356: assertTrue(A is|is not B) or assertFalse(A is|is not B) sentences must not be used. Use assertIs(A, B) or assertIsNot(A, B) instead')
...
@p.trace('return_followed_by_space')
@core.flake8ext
def return_followed_by_space(logical_line):
    ...
    if return_not_followed_by_space.match(logical_line):
        yield (0, 'N358: Return keyword should be followed by a space.')

@p.trace('no_redundant_import_alias')
@core.flake8ext
...
@p.trace('yield_followed_by_space')
@core.flake8ext
def yield_followed_by_space(logical_line):
    ...
    if yield_not_followed_by_space.match(logical_line):
        yield (0, 'N360: Yield keyword should be followed by a space.')

@p.trace('assert_regexpmatches')
@core.flake8ext
def assert_regexpmatches(logical_line):
    ...
    if res:
        yield (0, 'N361: assertRegex/assertNotRegex must be used instead of assertRegexpMatches/assertNotRegexpMatches.')

@p.trace('privsep_imports_not_aliased')
@core.flake8ext
def privsep_imports_not_aliased(logical_line, filename):
    ...
    N362
    """
    if not privsep_file_re.match(filename) and privsep_import_re.match(logical_line) and (logical_line.count(' ') > 1):
        yield (0, "N362: always import privsep modules so that the use of escalated permissions is obvious to callers. For example, use 'import nova.privsep.path' instead of 'from nova.privsep import path'.")

@p.trace('did_you_mean_tuple')
@core.flake8ext
def did_you_mean_tuple(logical_line):
    ...
    if disguised_as_tuple_re.search(logical_line):
        yield (0, 'N363: You said ``in (not_a_tuple)`` when you almost certainly meant ``in (a_tuple_of_one,)``.')

@p.trace('nonexistent_assertion_methods_and_attributes')
@core.flake8ext
def nonexistent_assertion_methods_and_attributes(logical_line, filename):
    ...
    N364
    """
    msg = "N364: Non existent mock assertion method or attribute (%s) is used. Check a typo or whether the assertion method should begin with 'assert_'."
    if 'nova/tests/' in filename:
...
@p.trace('check_assert_has_calls')
@core.flake8ext
def check_assert_has_calls(logical_line, filename):
    ...
    if 'nova/tests/' in filename and mock_assert_has_calls_re.search(logical_line):
        yield (0, msg)