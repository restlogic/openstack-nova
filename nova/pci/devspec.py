...
class PciAddressSpec(metaclass=abc.ABCMeta):
    ...
    def is_single_address(self):
        return all([all((c in string.hexdigits for c in self.domain)), all((c in string.hexdigits for c in self.bus)), all((c in string.hexdigits for c in self.slot)), all((c in string.hexdigits for c in self.func))])
    ...
    def _set_pci_dev_info(self, prop, maxval, hex_value):
        ...
        try:
            ...
            raise exception.PciConfigInvalidWhitelist(reason=_("property %(property)s ('%(attr)s') does not parse as a hex number.") % {'property': prop, 'attr': a})
        if v > maxval:
            raise exception.PciConfigInvalidWhitelist(reason=_('property %(property)s (%(attr)s) is greater than the maximum allowable value (%(max)X).') % {'property': prop, 'attr': a, 'max': maxval})
        setattr(self, prop, hex_value % v)

@p.trace_cls('PhysicalPciAddress')
class PhysicalPciAddress(PciAddressSpec):
    ...
    def __init__(self, pci_addr):
        try:
            if isinstance(pci_addr, dict):
                ...
                (self.domain, self.bus, self.slot, self.func) = utils.get_pci_address_fields(pci_addr)
    ...
    def match(self, phys_pci_addr):
        conditions = [self.domain == phys_pci_addr.domain, self.bus == phys_pci_addr.bus, self.slot == phys_pci_addr.slot, self.func == phys_pci_addr.func]
        return all(conditions)

@p.trace_cls('PciAddressGlobSpec')
class PciAddressGlobSpec(PciAddressSpec):
    ...
    def match(self, phys_pci_addr):
        conditions = [self.domain in (ANY, phys_pci_addr.domain), self.bus in (ANY, phys_pci_addr.bus), self.slot in (ANY, phys_pci_addr.slot), self.func in (ANY, phys_pci_addr.func)]
        return all(conditions)

@p.trace_cls('PciAddressRegexSpec')
class PciAddressRegexSpec(PciAddressSpec):
    ...
    def match(self, phys_pci_addr):
        conditions = [bool(self.domain_regex.match(phys_pci_addr.domain)), bool(self.bus_regex.match(phys_pci_addr.bus)), bool(self.slot_regex.match(phys_pci_addr.slot)), bool(self.func_regex.match(phys_pci_addr.func))]
        return all(conditions)

@p.trace_cls('WhitelistPciAddress')
class WhitelistPciAddress(object):
    ...
    def _check_physical_function(self):
        if self.pci_address_spec.is_single_address():
            self.is_physical_function = utils.is_physical_function(self.pci_address_spec.domain, self.pci_address_spec.bus, self.pci_address_spec.slot, self.pci_address_spec.func)
...
class PciDeviceSpec(PciAddressSpec):
    ...
    def match(self, dev_dict):
        ...
        return all([self.vendor_id in (ANY, dev_dict['vendor_id']), self.product_id in (ANY, dev_dict['product_id']), address_obj.match(dev_dict['address'], dev_dict.get('parent_addr'))])

    def match_pci_obj(self, pci_obj):
        return self.match({'vendor_id': pci_obj.vendor_id, 'product_id': pci_obj.product_id, 'address': pci_obj.address, 'parent_addr': pci_obj.parent_addr})
