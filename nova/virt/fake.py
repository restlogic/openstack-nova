...
class Resources(object):
    ...
    def dump(self):
        return {'vcpus': self.vcpus, 'memory_mb': self.memory_mb, 'local_gb': self.local_gb, 'vcpus_used': self.vcpus_used, 'memory_mb_used': self.memory_mb_used, 'local_gb_used': self.local_gb_used}

@p.trace_cls('FakeDriver')
class FakeDriver(driver.ComputeDriver):
    capabilities = {'has_imagecache': True, 'supports_evacuate': True, 'supports_migrate_to_same_host': False, 'supports_attach_interface': True, 'supports_device_tagging': True, 'supports_tagged_attach_interface': True, 'supports_tagged_attach_volume': True, 'supports_extend_volume': True, 'supports_multiattach': True, 'supports_trusted_certs': True, 'supports_pcpus': False, 'supports_accelerators': True, 'supports_image_type_raw': True, 'supports_image_type_vhd': False}
    vcpus = 1000
    memory_mb = 800000
    local_gb = 600000
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        self.host_status_base = {'hypervisor_type': 'fake', 'hypervisor_version': versionutils.convert_version_to_int('1.0'), 'hypervisor_hostname': CONF.host, 'cpu_info': {}, 'disk_available_least': 0, 'supported_instances': [(obj_fields.Architecture.X86_64, obj_fields.HVType.FAKE, obj_fields.VMMode.HVM)], 'numa_topology': None}
        self._mounts = {}
        self._interfaces = {}
    ...
    def init_host(self, host):
        self._host = host
        self._nodes = ['fake-mini'] if self._host == 'compute' else [self._host]
    ...
    def reboot(self, context, instance, network_info, reboot_type, block_device_info=None, bad_volumes_callback=None, accel_info=None):
        if instance.uuid not in self.instances and reboot_type.lower() == 'hard':
            injected_files = admin_password = allocations = None
            self.spawn(context, instance, instance.image_meta, injected_files, admin_password, allocations, block_device_info=block_device_info)
        else:
            self.power_on(context, instance, network_info, block_device_info=block_device_info)
    ...
    def finish_revert_migration(self, context, instance, network_info, migration, block_device_info=None, power_on=True):
        state = power_state.RUNNING if power_on else power_state.SHUTDOWN
        self.instances[instance.uuid] = FakeInstance(instance.name, state, instance.uuid)
    ...
    def post_live_migration_at_destination(self, context, instance, network_info, block_migration=False, block_device_info=None):
        ...
        self.spawn(context, instance, image_meta, injected_files, admin_password, allocations)
    ...
    def destroy(self, context, instance, network_info, block_device_info=None, destroy_disks=True, destroy_secrets=True):
        ...
        if key in self.instances:
            flavor = instance.flavor
            self.resources.release(vcpus=flavor.vcpus, mem=flavor.memory_mb, disk=flavor.root_gb)
            del self.instances[key]
        else:
            LOG.warning("Key '%(key)s' not in instances '%(inst)s'", {'key': key, 'inst': self.instances}, instance=instance)
    ...
    def cleanup(self, context, instance, network_info, block_device_info=None, destroy_disks=True, migrate_data=None, destroy_vifs=True, destroy_secrets=True):
        if instance.uuid in self.instances:
            raise exception.InstanceExists('Instance %s has not been destroyed.' % instance.uuid)
    ...
    def get_diagnostics(self, instance):
        return {'cpu0_time': 17300000000, 'memory': 524288, 'vda_errors': -1, 'vda_read': 262144, 'vda_read_req': 112, 'vda_write': 5778432, 'vda_write_req': 488, 'vnet1_rx': 2070139, 'vnet1_rx_drop': 0, 'vnet1_rx_errors': 0, 'vnet1_rx_packets': 26701, 'vnet1_tx': 140208, 'vnet1_tx_drop': 0, 'vnet1_tx_errors': 0, 'vnet1_tx_packets': 662}

    def get_instance_diagnostics(self, instance):
        diags = diagnostics_obj.Diagnostics(state='running', driver='libvirt', hypervisor='kvm', hypervisor_os='ubuntu', uptime=46664, config_drive=True)
        diags.add_cpu(id=0, time=17300000000, utilisation=15)
        diags.add_nic(mac_address='01:23:45:67:89:ab', rx_octets=2070139, rx_errors=100, rx_drop=200, rx_packets=26701, rx_rate=300, tx_octets=140208, tx_errors=400, tx_drop=500, tx_packets=662, tx_rate=600)
        diags.add_disk(read_bytes=262144, read_requests=112, write_bytes=5778432, write_requests=488, errors_count=1)
        diags.memory_details = diagnostics_obj.MemoryDiagnostics(maximum=524288, used=0)
        return diags
    ...
    def get_all_volume_usage(self, context, compute_host_bdms):
        ...
        if compute_host_bdms:
            volusage = [{'volume': compute_host_bdms[0]['instance_bdms'][0]['volume_id'], 'instance': compute_host_bdms[0]['instance'], 'rd_bytes': 0, 'rd_req': 0, 'wr_bytes': 0, 'wr_req': 0}]
        return volusage

    def get_host_cpu_stats(self):
        stats = {'kernel': 5664160000000, 'idle': 1592705190000000, 'user': 26728850000000, 'iowait': 6121490000000}
        stats['frequency'] = 800
        return stats
    ...
    def get_vnc_console(self, context, instance):
        return ctype.ConsoleVNC(internal_access_path='FAKE', host='fakevncconsole.com', port=6969)

    def get_spice_console(self, context, instance):
        return ctype.ConsoleSpice(internal_access_path='FAKE', host='fakespiceconsole.com', port=6969, tlsPort=6970)

    def get_rdp_console(self, context, instance):
        return ctype.ConsoleRDP(internal_access_path='FAKE', host='fakerdpconsole.com', port=6969)

    def get_serial_console(self, context, instance):
        return ctype.ConsoleSerial(internal_access_path='FAKE', host='fakerdpconsole.com', port=6969)

    def get_mks_console(self, context, instance):
        return ctype.ConsoleMKS(internal_access_path='FAKE', host='fakemksconsole.com', port=6969)
    ...
    def get_available_resource(self, nodename):
        """Updates compute manager resource info on ComputeNode table.
           ...
           disk and ram.
        """
        cpu_info = collections.OrderedDict([('arch', 'x86_64'), ('model', 'Nehalem'), ('vendor', 'Intel'), ('features', ['pge', 'clflush']), ('topology', {'cores': 1, 'threads': 1, 'sockets': 4})])
    ...
    def finish_migration(self, context, migration, instance, disk_info, network_info, image_meta, resize_instance, allocations, block_device_info=None, power_on=True):
        ...
        self.spawn(context, instance, image_meta, injected_files, admin_password, allocations, block_device_info=block_device_info, power_on=power_on)
    ...
    def get_volume_connector(self, instance):
        return {'ip': CONF.my_block_storage_ip, 'initiator': 'fake', 'host': self._host}
...
@p.trace_cls('MediumFakeDriverWithNestedCustomResources')
class MediumFakeDriverWithNestedCustomResources(MediumFakeDriver):
    vcpus = 10
    memory_mb = 8192
    local_gb = 1028
    child_resources = {'CUSTOM_MAGIC': {'total': 10, 'reserved': 0, 'min_unit': 1, 'max_unit': 10, 'step_size': 1, 'allocation_ratio': 1}}

    def update_provider_tree(self, provider_tree, nodename, allocations=None):
        super(MediumFakeDriverWithNestedCustomResources, self).update_provider_tree(provider_tree, nodename, allocations=allocations)
        if not provider_tree.exists(nodename + '-child'):
            provider_tree.new_child(name=nodename + '-child', parent=nodename)
        provider_tree.update_inventory(nodename + '-child', self.child_resources)

@p.trace_cls('FakeFinishMigrationFailDriver')
...
@p.trace_cls('PredictableNodeUUIDDriver')
class PredictableNodeUUIDDriver(SmallFakeDriver):
    ...
    def get_available_resource(self, nodename):
        resources = super(PredictableNodeUUIDDriver, self).get_available_resource(nodename)
        resources['uuid'] = uuid.uuid5(uuid.NAMESPACE_DNS, nodename)
        return resources

@p.trace_cls('FakeRescheduleDriver')
class FakeRescheduleDriver(FakeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files, admin_password, allocations, network_info=None, block_device_info=None, power_on=True, accel_info=None):
        if not self.rescheduled.get(instance.uuid, False):
            ...
            raise exception.ComputeResourcesUnavailable(reason='FakeRescheduleDriver')
        super(FakeRescheduleDriver, self).spawn(context, instance, image_meta, injected_files, admin_password, allocations, network_info, block_device_info, power_on)

@p.trace_cls('FakeRescheduleDriverWithNestedCustomResources')
class FakeRescheduleDriverWithNestedCustomResources(FakeRescheduleDriver, MediumFakeDriverWithNestedCustomResources):
    pass

@p.trace_cls('FakeBuildAbortDriver')
class FakeBuildAbortDriver(FakeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files, admin_password, allocations, network_info=None, block_device_info=None, power_on=True, accel_info=None):
        raise exception.BuildAbortException(instance_uuid=instance.uuid, reason='FakeBuildAbortDriver')

@p.trace_cls('FakeBuildAbortDriverWithNestedCustomResources')
class FakeBuildAbortDriverWithNestedCustomResources(FakeBuildAbortDriver, MediumFakeDriverWithNestedCustomResources):
    pass

@p.trace_cls('FakeUnshelveSpawnFailDriver')
class FakeUnshelveSpawnFailDriver(FakeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files, admin_password, allocations, network_info=None, block_device_info=None, power_on=True, accel_info=None):
        if instance.vm_state == vm_states.SHELVED_OFFLOADED:
            raise exception.VirtualInterfaceCreateException('FakeUnshelveSpawnFailDriver')
        super(FakeUnshelveSpawnFailDriver, self).spawn(context, instance, image_meta, injected_files, admin_password, allocations, network_info, block_device_info, power_on)

@p.trace_cls('FakeUnshelveSpawnFailDriverWithNestedCustomResources')
class FakeUnshelveSpawnFailDriverWithNestedCustomResources(FakeUnshelveSpawnFailDriver, MediumFakeDriverWithNestedCustomResources):
    pass

@p.trace_cls('FakeLiveMigrateDriver')
class FakeLiveMigrateDriver(FakeDriver):
    ...
    def post_live_migration(self, context, instance, block_device_info, migrate_data=None):
        self.destroy(context, instance, network_info=None, block_device_info=block_device_info)

@p.trace_cls('FakeLiveMigrateDriverWithNestedCustomResources')
class FakeLiveMigrateDriverWithNestedCustomResources(FakeLiveMigrateDriver, MediumFakeDriverWithNestedCustomResources):
    pass

@p.trace_cls('FakeDriverWithPciResources')
...
@p.trace_cls('FakeDriverWithCaching')
