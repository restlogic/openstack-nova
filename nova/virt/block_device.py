...
@p.trace_cls('_NotTransformable')
class _NotTransformable(Exception):
    pass

@p.trace_cls('_InvalidType')
...
@p.trace('_get_volume_create_az_value')
...
class DriverBlockDevice(dict):
    ...
    _update_on_save = {'disk_bus': None, 'device_name': None, 'device_type': None}
...
@p.trace_cls('DriverSwapBlockDevice')
class DriverSwapBlockDevice(DriverBlockDevice):
    ...
    _update_on_save = {'disk_bus': None, 'device_name': None}

    def _transform(self):
        if not block_device.new_format_is_swap(self._bdm_obj):
            raise _InvalidType
        self.update({'device_name': self._bdm_obj.device_name, 'swap_size': self._bdm_obj.volume_size or 0, 'disk_bus': self._bdm_obj.disk_bus})

@p.trace_cls('DriverEphemeralBlockDevice')
class DriverEphemeralBlockDevice(DriverBlockDevice):
    ...
    _legacy_fields = _fields - _new_only_fields | set(['num', 'virtual_name'])

    def _transform(self):
        if not block_device.new_format_is_ephemeral(self._bdm_obj):
            raise _InvalidType
        self.update({'device_name': self._bdm_obj.device_name, 'size': self._bdm_obj.volume_size or 0, 'disk_bus': self._bdm_obj.disk_bus, 'device_type': self._bdm_obj.device_type, 'guest_format': self._bdm_obj.guest_format})
...
@p.trace_cls('DriverVolumeBlockDevice')
class DriverVolumeBlockDevice(DriverBlockDevice):
    _legacy_fields = set(['connection_info', 'mount_device', 'delete_on_termination'])
    _new_fields = set(['guest_format', 'device_type', 'disk_bus', 'boot_index', 'attachment_id'])
    ...
    _update_on_save = {'disk_bus': None, 'device_name': 'mount_device', 'device_type': None, 'attachment_id': None}

    def _transform(self):
        if not self._bdm_obj.source_type == self._valid_source or not self._bdm_obj.destination_type == self._valid_destination:
            raise _InvalidType
        self.update({k: v for (k, v) in self._bdm_obj.items() if k in self._new_fields | set(['delete_on_termination'])})
        self['mount_device'] = self._bdm_obj.device_name
        try:
            self['connection_info'] = jsonutils.loads(self._bdm_obj.connection_info)
        except TypeError:
            self['connection_info'] = {}
        self['volume_type'] = self.volume_type if 'volume_type' in self._bdm_obj else None
    ...
    def _preserve_multipath_id(self, connection_info):
        if self['connection_info'] and 'data' in self['connection_info']:
            if 'multipath_id' in self['connection_info']['data']:
                connection_info['data']['multipath_id'] = self['connection_info']['data']['multipath_id']
                LOG.info('preserve multipath_id %s', connection_info['data']['multipath_id'])

    def driver_detach(self, context, instance, volume_api, virt_driver):
        connection_info = self['connection_info']
        mp = self['mount_device']
        volume_id = self.volume_id
        LOG.info('Attempting to driver detach volume %(volume_id)s from mountpoint %(mp)s', {'volume_id': volume_id, 'mp': mp}, instance=instance)
        try:
            if not virt_driver.instance_exists(instance):
                LOG.warning('Detaching volume from unknown instance', instance=instance)
            encryption = encryptors.get_encryption_metadata(context, volume_api, volume_id, connection_info)
            virt_driver.detach_volume(context, connection_info, instance, mp, encryption=encryption)
        except exception.DiskNotFound as err:
            LOG.warning('Ignoring DiskNotFound exception while detaching volume %(volume_id)s from %(mp)s : %(err)s', {'volume_id': volume_id, 'mp': mp, 'err': err}, instance=instance)
        except exception.DeviceDetachFailed:
            with excutils.save_and_reraise_exception():
                LOG.warning('Guest refused to detach volume %(vol)s', {'vol': volume_id}, instance=instance)
                volume_api.roll_detaching(context, volume_id)
        except Exception:
            with excutils.save_and_reraise_exception():
                LOG.exception('Failed to detach volume %(volume_id)s from %(mp)s', {'volume_id': volume_id, 'mp': mp}, instance=instance)
                volume_api.roll_detaching(context, volume_id)
    ...
    def _create_volume(self, context, instance, volume_api, size, wait_func=None, **create_kwargs):
        ...
        vol = volume_api.create(context, size, name, description, volume_type=self.volume_type, availability_zone=av_zone, **create_kwargs)
        if wait_func:
            self._call_wait_func(context, wait_func, volume_api, vol['id'])
        attachment_id = volume_api.attachment_create(context, vol['id'], instance.uuid)['id']
        return (vol['id'], attachment_id)

    def _do_detach(self, context, instance, volume_api, virt_driver, attachment_id=None, destroy_bdm=False):
        ...
        if CONF.host == instance.host:
            ...
            LOG.debug('Skipping driver_detach during remote rebuild.', instance=instance)
        elif destroy_bdm:
            LOG.error('Unable to call for a driver detach of volume %(vol_id)s due to the instance being registered to the remote host %(inst_host)s.', {'vol_id': volume_id, 'inst_host': instance.host}, instance=instance)
        if not self['attachment_id']:
            ...
            if connection_info and (not destroy_bdm) and (connector.get('host') != instance.host):
                ...
                if not stashed_connector:
                    LOG.warning('Host mismatch detected, but stashed volume connector not found. Instance host is %(ihost)s, but volume connector host is %(chost)s.', {'ihost': instance.host, 'chost': connector.get('host')})
                elif stashed_connector.get('host') != instance.host:
                    LOG.error('Host mismatch detected in stashed volume connector. Will use local volume connector. Instance host is %(ihost)s. Local volume connector host is %(chost)s. Stashed volume connector host is %(schost)s.', {'ihost': instance.host, 'chost': connector.get('host'), 'schost': stashed_connector.get('host')})
                else:
                    LOG.debug('Host mismatch detected. Found usable stashed volume connector. Instance host is %(ihost)s. Local volume connector host was %(chost)s. Stashed volume connector host is %(schost)s.', {'ihost': instance.host, 'chost': connector.get('host'), 'schost': stashed_connector.get('host')})
                    connector = stashed_connector
            volume_api.terminate_connection(context, volume_id, connector)
            volume_api.detach(context.elevated(), volume_id, instance.uuid, attachment_id)
        else:
            volume_api.attachment_delete(context, self['attachment_id'])

    def detach(self, context, instance, volume_api, virt_driver, attachment_id=None, destroy_bdm=False):
        volume = self._get_volume(context, volume_api, self.volume_id)
        with brick_utils.guard_connection(volume):
            self._do_detach(context, instance, volume_api, virt_driver, attachment_id, destroy_bdm)

    def _legacy_volume_attach(self, context, volume, connector, instance, volume_api, virt_driver, do_driver_attach=False):
        volume_id = volume['id']
        ...
        if volume['attach_status'] == 'detached':
            ...
            try:
                ...
                with excutils.save_and_reraise_exception():
                    if do_driver_attach:
                        try:
                            virt_driver.detach_volume(context, connection_info, instance, self['mount_device'], encryption=encryption)
                        except Exception:
                            LOG.warning('Driver failed to detach volume %(volume_id)s at %(mount_point)s.', {'volume_id': volume_id, 'mount_point': self['mount_device']}, exc_info=True, instance=instance)
                    volume_api.terminate_connection(context, volume_id, connector)
                    volume_api.detach(context, volume_id)

    def _volume_attach(self, context, volume, connector, instance, volume_api, virt_driver, attachment_id, do_driver_attach=False):
        volume_id = volume['id']
        if self.volume_size is None:
            self.volume_size = volume.get('size')
        vol_multiattach = volume.get('multiattach', False)
        virt_multiattach = virt_driver.capabilities.get('supports_multiattach', False)
        if vol_multiattach and (not virt_multiattach):
            raise exception.MultiattachNotSupportedByVirtDriver(volume_id=volume_id)
        LOG.debug('Updating existing volume attachment record: %s', attachment_id, instance=instance)
        connection_info = volume_api.attachment_update(context, attachment_id, connector, self['mount_device'])['connection_info']
        if 'serial' not in connection_info:
            connection_info['serial'] = self.volume_id
        self._preserve_multipath_id(connection_info)
        if vol_multiattach:
    ...
    def _do_attach(self, context, instance, volume, volume_api, virt_driver, do_driver_attach):
        ...
        connector = virt_driver.get_volume_connector(instance)
        if not self['attachment_id']:
            self._legacy_volume_attach(context, volume, connector, instance, volume_api, virt_driver, do_driver_attach)
        else:
            self._volume_attach(context, volume, connector, instance, volume_api, virt_driver, self['attachment_id'], do_driver_attach)
    ...
    def attach(self, context, instance, volume_api, virt_driver, do_driver_attach=False, **kwargs):
        ...
        with brick_utils.guard_connection(volume):
            self._do_attach(context, instance, volume, volume_api, virt_driver, do_driver_attach)
    ...
    def refresh_connection_info(self, context, instance, volume_api, virt_driver):
        ...
        if not self['attachment_id']:
            ...
            attachment_ref = volume_api.attachment_get(context, self['attachment_id'])
    ...
    def save(self):
        try:
            connection_info_string = jsonutils.dumps(self.get('connection_info'))
            ...
            with excutils.save_and_reraise_exception():
                if self['delete_on_termination']:
                    try:
                        ...
                        LOG.warning('Failed to delete volume: %(volume_id)s due to %(exc)s', {'volume_id': volume_id, 'exc': exc})

@p.trace_cls('DriverVolSnapshotBlockDevice')
class DriverVolSnapshotBlockDevice(DriverVolumeBlockDevice):
    ...
    def attach(self, context, instance, volume_api, virt_driver, wait_func=None):
        if not self.volume_id:
            ...
            (self.volume_id, self.attachment_id) = self._create_volume(context, instance, volume_api, self.volume_size, wait_func=wait_func, snapshot=snapshot)
        super(DriverVolSnapshotBlockDevice, self).attach(context, instance, volume_api, virt_driver)

@p.trace_cls('DriverVolImageBlockDevice')
class DriverVolImageBlockDevice(DriverVolumeBlockDevice):
    ...
    def attach(self, context, instance, volume_api, virt_driver, wait_func=None):
        if not self.volume_id:
            (self.volume_id, self.attachment_id) = self._create_volume(context, instance, volume_api, self.volume_size, wait_func=wait_func, image_id=self.image_id)
        super(DriverVolImageBlockDevice, self).attach(context, instance, volume_api, virt_driver)

@p.trace_cls('DriverVolBlankBlockDevice')
class DriverVolBlankBlockDevice(DriverVolumeBlockDevice):
    ...
    def attach(self, context, instance, volume_api, virt_driver, wait_func=None):
        if not self.volume_id:
            ...
            (self.volume_id, self.attachment_id) = self._create_volume(context, instance, volume_api, self.volume_size, wait_func=wait_func, name=vol_name)
        super(DriverVolBlankBlockDevice, self).attach(context, instance, volume_api, virt_driver)

@p.trace('_convert_block_devices')
def _convert_block_devices(device_type, block_device_mapping):
    ...
    for bdm in block_device_mapping:
        try:
            ...
            pass
    return devices
convert_swap = functools.partial(_convert_block_devices, DriverSwapBlockDevice)
convert_ephemerals = functools.partial(_convert_block_devices, DriverEphemeralBlockDevice)
convert_volumes = functools.partial(_convert_block_devices, DriverVolumeBlockDevice)
convert_snapshots = functools.partial(_convert_block_devices, DriverVolSnapshotBlockDevice)
convert_images = functools.partial(_convert_block_devices, DriverVolImageBlockDevice)
convert_blanks = functools.partial(_convert_block_devices, DriverVolBlankBlockDevice)

@p.trace('convert_all_volumes')
def convert_all_volumes(*volume_bdms):
    ...
    return [vol for vol in itertools.chain(source_volume, source_snapshot, source_image, source_blank)]

@p.trace('convert_volume')
...
@p.trace('attach_block_devices')
def attach_block_devices(block_device_mapping, *attach_args, **attach_kwargs):

    def _log_and_attach(bdm):
        instance = attach_args[1]
        if bdm.get('volume_id'):
            LOG.info('Booting with volume %(volume_id)s at %(mountpoint)s', {'volume_id': bdm.volume_id, 'mountpoint': bdm['mount_device']}, instance=instance)
        elif bdm.get('snapshot_id'):
            LOG.info('Booting with volume snapshot %(snapshot_id)s at %(mountpoint)s', {'snapshot_id': bdm.snapshot_id, 'mountpoint': bdm['mount_device']}, instance=instance)
        elif bdm.get('image_id'):
            LOG.info('Booting with volume-backed-image %(image_id)s at %(mountpoint)s', {'image_id': bdm.image_id, 'mountpoint': bdm['mount_device']}, instance=instance)
        else:
            LOG.info('Booting with blank volume at %(mountpoint)s', {'mountpoint': bdm['mount_device']}, instance=instance)
        bdm.attach(*attach_args, **attach_kwargs)
    for device in block_device_mapping:
        _log_and_attach(device)
    return block_device_mapping

@p.trace('refresh_conn_infos')
...
@p.trace('legacy_block_devices')
def legacy_block_devices(block_device_mapping):
    bdms = [bdm.legacy() for bdm in block_device_mapping]
    if all((isinstance(bdm, DriverEphemeralBlockDevice) for bdm in block_device_mapping)):
...
def get_swap(transformed_list):
    ...
    """
    if not all((isinstance(device, DriverSwapBlockDevice) or 'swap_size' in device for device in transformed_list)):
        return None
    try:
        return transformed_list.pop()
    except IndexError:
        return None
_IMPLEMENTED_CLASSES = (DriverSwapBlockDevice, DriverEphemeralBlockDevice, DriverVolumeBlockDevice, DriverVolSnapshotBlockDevice, DriverVolImageBlockDevice, DriverVolBlankBlockDevice)

@p.trace('is_implemented')
...
@p.trace('is_block_device_mapping')
def is_block_device_mapping(bdm):
    return bdm.source_type in ('image', 'volume', 'snapshot', 'blank') and bdm.destination_type == 'volume' and is_implemented(bdm)

@p.trace('get_volume_id')
