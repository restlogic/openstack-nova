...
@p.trace_cls('VTPMConfig')
class VTPMConfig(ty.NamedTuple):
    version: str
    model: str

@p.trace('get_vcpu_pin_set')
...
def get_cpu_dedicated_set():
    ...
    if not cpu_ids:
        msg = _("No CPUs available after parsing '[compute] cpu_dedicated_set' config, %r")
        raise exception.Invalid(msg % CONF.compute.cpu_dedicated_set)
    return cpu_ids

@p.trace('get_cpu_shared_set')
def get_cpu_shared_set():
    ...
    if not shared_ids:
        msg = _("No CPUs available after parsing '[compute] cpu_shared_set' config, %r")
        raise exception.Invalid(msg % CONF.compute.cpu_shared_set)
    return shared_ids

@p.trace('parse_cpu_spec')
    ...
    for rule in spec.split(','):
        ...
        if len(range_parts) > 1:
            ...
            try:
                ...
                raise exception.Invalid(_('Invalid exclusion expression %r') % rule)
        else:
            try:
                cpuset_ids.add(int(rule))
            except ValueError:
                raise exception.Invalid(_('Invalid inclusion expression %r') % rule)
    cpuset_ids -= cpuset_reject_ids
    return cpuset_ids

@p.trace('format_cpu_spec')
def format_cpu_spec(cpuset: ty.Set[int], allow_ranges: bool=True) -> str:
...
@p.trace('get_number_of_serial_ports')
def get_number_of_serial_ports(flavor, image_meta):
    ...
    """
    (flavor_num_ports, image_num_ports) = _get_flavor_image_meta('serial_port_count', flavor, image_meta)
    if flavor_num_ports:
        try:
            flavor_num_ports = int(flavor_num_ports)
        except ValueError:
            raise exception.ImageSerialPortNumberInvalid(num_ports=flavor_num_ports)
...
@p.trace('get_cpu_topology_constraints')
def get_cpu_topology_constraints(flavor, image_meta):
    ...
    """
    (flavor_max_sockets, image_max_sockets) = _get_flavor_image_meta('cpu_max_sockets', flavor, image_meta, 0)
    (flavor_max_cores, image_max_cores) = _get_flavor_image_meta('cpu_max_cores', flavor, image_meta, 0)
    (flavor_max_threads, image_max_threads) = _get_flavor_image_meta('cpu_max_threads', flavor, image_meta, 0)
    try:
    ...
    LOG.debug('Flavor limits %(sockets)d:%(cores)d:%(threads)d', {'sockets': flavor_max_sockets, 'cores': flavor_max_cores, 'threads': flavor_max_threads})
    LOG.debug('Image limits %(sockets)d:%(cores)d:%(threads)d', {'sockets': image_max_sockets, 'cores': image_max_cores, 'threads': image_max_threads})
    if flavor_max_sockets and image_max_sockets > flavor_max_sockets or (flavor_max_cores and image_max_cores > flavor_max_cores) or (flavor_max_threads and image_max_threads > flavor_max_threads):
        raise exception.ImageVCPULimitsRangeExceeded(image_sockets=image_max_sockets, image_cores=image_max_cores, image_threads=image_max_threads, flavor_sockets=flavor_max_sockets, flavor_cores=flavor_max_cores, flavor_threads=flavor_max_threads)
    ...
    (flavor_sockets, image_sockets) = _get_flavor_image_meta('cpu_sockets', flavor, image_meta, 0)
    (flavor_cores, image_cores) = _get_flavor_image_meta('cpu_cores', flavor, image_meta, 0)
    (flavor_threads, image_threads) = _get_flavor_image_meta('cpu_threads', flavor, image_meta, 0)
    try:
        ...
        msg = _('Invalid flavor extra spec. Error: %s') % str(e)
        raise exception.InvalidRequest(msg)
    LOG.debug('Flavor pref %(sockets)d:%(cores)d:%(threads)d', {'sockets': flavor_sockets, 'cores': flavor_cores, 'threads': flavor_threads})
    LOG.debug('Image pref %(sockets)d:%(cores)d:%(threads)d', {'sockets': image_sockets, 'cores': image_cores, 'threads': image_threads})
    if flavor_sockets > max_sockets or flavor_cores > max_cores or flavor_threads > max_threads:
        flavor_sockets = flavor_cores = flavor_threads = 0
    if image_sockets > max_sockets or image_cores > max_cores or image_threads > max_threads:
        raise exception.ImageVCPUTopologyRangeExceeded(image_sockets=image_sockets, image_cores=image_cores, image_threads=image_threads, max_sockets=max_sockets, max_cores=max_cores, max_threads=max_threads)
    ...
    LOG.debug('Chose sockets=%(sockets)d, cores=%(cores)d, threads=%(threads)d; limits were sockets=%(maxsockets)d, cores=%(maxcores)d, threads=%(maxthreads)d', {'sockets': sockets, 'cores': cores, 'threads': threads, 'maxsockets': max_sockets, 'maxcores': max_cores, 'maxthreads': max_threads})
    return (objects.VirtCPUTopology(sockets=sockets, cores=cores, threads=threads), objects.VirtCPUTopology(sockets=max_sockets, cores=max_cores, threads=max_threads))

@p.trace('_get_possible_cpu_topologies')
def _get_possible_cpu_topologies(vcpus, maxtopology, allow_threads):
    ...
    if not allow_threads:
        maxthreads = 1
    LOG.debug('Build topologies for %(vcpus)d vcpu(s) %(maxsockets)d:%(maxcores)d:%(maxthreads)d', {'vcpus': vcpus, 'maxsockets': maxsockets, 'maxcores': maxcores, 'maxthreads': maxthreads})
    possible = []
    for s in range(1, maxsockets + 1):
        for c in range(1, maxcores + 1):
            for t in range(1, maxthreads + 1):
                if t * c * s != vcpus:
                    continue
                possible.append(objects.VirtCPUTopology(sockets=s, cores=c, threads=t))
    possible = sorted(possible, reverse=True, key=lambda x: (x.sockets * x.cores, x.sockets, x.threads))
    LOG.debug('Got %d possible topologies', len(possible))
    if len(possible) == 0:
        raise exception.ImageVCPULimitsRangeImpossible(vcpus=vcpus, sockets=maxsockets, cores=maxcores, threads=maxthreads)
    return possible

@p.trace('_sort_possible_cpu_topologies')
def _sort_possible_cpu_topologies(possible, wanttopology):
    ...
    """
    scores: ty.Dict[int, ty.List['objects.VirtCPUTopology']] = collections.defaultdict(list)
...
@p.trace('_get_desirable_cpu_topologies')
def _get_desirable_cpu_topologies(flavor, image_meta, allow_threads=True):
    ...
    """
    LOG.debug('Getting desirable topologies for flavor %(flavor)s and image_meta %(image_meta)s, allow threads: %(threads)s', {'flavor': flavor, 'image_meta': image_meta, 'threads': allow_threads})
    (preferred, maximum) = get_cpu_topology_constraints(flavor, image_meta)
    LOG.debug('Topology preferred %(preferred)s, maximum %(maximum)s', {'preferred': preferred, 'maximum': maximum})
    possible = _get_possible_cpu_topologies(flavor.vcpus, maximum, allow_threads)
...
@p.trace('_numa_cell_supports_pagesize_request')
...
@p.trace('_pack_instance_onto_cores')
def _pack_instance_onto_cores(host_cell, instance_cell, num_cpu_reserved=0):
    ...
    LOG.debug('Packing an instance onto a set of siblings:     host_cell_free_siblings: %(siblings)s    instance_cell: %(cells)s    host_cell_id: %(host_cell_id)s    threads_per_core: %(threads_per_core)s    num_cpu_reserved: %(num_cpu_reserved)s', {'siblings': host_cell.free_siblings, 'cells': instance_cell, 'host_cell_id': host_cell.id, 'threads_per_core': threads_per_core, 'num_cpu_reserved': num_cpu_reserved})
    sibling_sets: ty.Dict[int, ty.List[ty.Set[int]]] = collections.defaultdict(list)
    ...
    def _get_pinning(threads_no, sibling_set, instance_cores):
        ...
        vcpus_pinning = list(zip(sorted(instance_cores), itertools.chain(*usable_cores)))
        msg = 'Computed NUMA topology CPU pinning: usable pCPUs: %(usable_cores)s, vCPUs mapping: %(vcpus_pinning)s'
        msg_args = {'usable_cores': usable_cores, 'vcpus_pinning': vcpus_pinning}
        LOG.info(msg, msg_args)
        return vcpus_pinning

    def _get_reserved(sibling_set, vcpus_pinning, num_cpu_reserved=0, cpu_thread_isolate=False):
                ...
                for sib in usable_cores:
                    if pcpu in sib:
                        if cpu_thread_isolate:
                            ...
                            sib.remove(pcpu)
            cpuset_reserved = set(list(itertools.chain(*usable_cores))[:num_cpu_reserved])
            msg = 'Computed NUMA topology reserved pCPUs: usable pCPUs: %(usable_cores)s, reserved pCPUs: %(cpuset_reserved)s'
            msg_args = {'usable_cores': usable_cores, 'cpuset_reserved': cpuset_reserved}
            LOG.info(msg, msg_args)
        return cpuset_reserved or None
    if instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.REQUIRE:
        LOG.debug("Requested 'require' thread policy for %d cores", len(instance_cell))
    elif instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.PREFER:
        LOG.debug("Requested 'prefer' thread policy for %d cores", len(instance_cell))
    elif instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.ISOLATE:
        LOG.debug("Requested 'isolate' thread policy for %d cores", len(instance_cell))
    else:
        LOG.debug('User did not specify a thread policy. Using default for %d cores', len(instance_cell))
    if instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.ISOLATE:
        if threads_per_core not in sibling_sets:
            LOG.debug('Host does not have any fully free thread sibling sets.It is not possible to emulate a non-SMT behavior for the isolate policy without this.')
            return
        if threads_per_core != 1 and host_cell.pcpuset != host_cell.cpuset:
            LOG.warning("Host supports hyperthreads, but instance requested no hyperthreads. This should have been rejected by the scheduler but we likely got here due to the fallback VCPU query. Consider setting '[workarounds] disable_fallback_pcpu_query' to 'True' once hosts are no longer using 'vcpu_pin_set'. Refer to bug #1889633 for more information.")
            return
        pinning = _get_pinning(1, sibling_sets[threads_per_core], instance_cell.pcpuset)
        cpuset_reserved = _get_reserved(sibling_sets[1], pinning, num_cpu_reserved=num_cpu_reserved, cpu_thread_isolate=True)
        if not pinning or (num_cpu_reserved and (not cpuset_reserved)):
            (pinning, cpuset_reserved) = (None, None)
    else:
        if instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.REQUIRE:
            if threads_per_core <= 1:
                LOG.info("Host does not support hyperthreading or hyperthreading is disabled, but 'require' threads policy was requested.")
                return
        for (threads_no, sibling_set) in sorted((t for t in sibling_sets.items()), reverse=True):
            if instance_cell.cpu_thread_policy == fields.CPUThreadAllocationPolicy.REQUIRE:
                if threads_no <= 1:
                    LOG.debug('Skipping threads_no: %s, as it does not satisfy the require policy', threads_no)
                    continue
            pinning = _get_pinning(threads_no, sibling_set, instance_cell.pcpuset)
            cpuset_reserved = _get_reserved(sibling_sets[1], pinning, num_cpu_reserved=num_cpu_reserved)
            if not pinning or (num_cpu_reserved and (not cpuset_reserved)):
                continue
            break
        if instance_cell.cpu_thread_policy != fields.CPUThreadAllocationPolicy.REQUIRE and (not pinning):
            sibling_set = [set([x]) for x in itertools.chain(*sibling_sets[1])]
            pinning = _get_pinning(threads_no, sibling_set, instance_cell.pcpuset)
            cpuset_reserved = _get_reserved(sibling_set, pinning, num_cpu_reserved=num_cpu_reserved)
    if not pinning or (num_cpu_reserved and (not cpuset_reserved)):
        return
    LOG.debug('Selected cores for pinning: %s, in cell %s', pinning, host_cell.id)
...
@p.trace('_numa_fit_instance_cell')
def _numa_fit_instance_cell(host_cell: 'objects.NUMACell', instance_cell: 'objects.InstanceNUMACell', limits: ty.Optional['objects.NUMATopologyLimit']=None, cpuset_reserved: int=0) -> ty.Optional['objects.InstanceNUMACell']:
    ...
    :returns: objects.InstanceNUMACell with the id set to that of the
              host, or None
    """
    LOG.debug('Attempting to fit instance cell %(cell)s on host_cell %(host_cell)s', {'cell': instance_cell, 'host_cell': host_cell})
    if 'pagesize' in instance_cell and instance_cell.pagesize:
        pagesize = _numa_cell_supports_pagesize_request(host_cell, instance_cell)
        if not pagesize:
            LOG.debug('Host does not support requested memory pagesize, or not enough free pages of the requested size. Requested: %d kB', instance_cell.pagesize)
            return None
        LOG.debug('Selected memory pagesize: %(selected_mem_pagesize)d kB. Requested memory pagesize: %(requested_mem_pagesize)d (small = -1, large = -2, any = -3)', {'selected_mem_pagesize': pagesize, 'requested_mem_pagesize': instance_cell.pagesize})
        instance_cell.pagesize = pagesize
    elif host_cell.mempages:
        pagesize = _get_smallest_pagesize(host_cell)
        LOG.debug('No specific pagesize requested for instance, selected pagesize: %d', pagesize)
        if not host_cell.can_fit_pagesize(pagesize, instance_cell.memory * units.Ki, use_free=False):
            LOG.debug('Not enough available memory to schedule instance with pagesize %(pagesize)d. Required: %(required)s, available: %(available)s, total: %(total)s.', {'required': instance_cell.memory, 'available': host_cell.avail_memory, 'total': host_cell.memory, 'pagesize': pagesize})
            return None
    elif instance_cell.memory > host_cell.memory:
        LOG.debug('Not enough host cell memory to fit instance cell. Required: %(required)d, actual: %(actual)d', {'required': instance_cell.memory, 'actual': host_cell.memory})
        return None
    if instance_cell.cpu_policy in (fields.CPUAllocationPolicy.DEDICATED, fields.CPUAllocationPolicy.MIXED):
        required_cpus = len(instance_cell.pcpuset) + cpuset_reserved
        if required_cpus > len(host_cell.pcpuset):
            LOG.debug('Not enough host cell CPUs to fit instance cell; required: %(required)d + %(cpuset_reserved)d as overhead, actual: %(actual)d', {'required': len(instance_cell.pcpuset), 'actual': len(host_cell.pcpuset), 'cpuset_reserved': cpuset_reserved})
            return None
    if instance_cell.cpu_policy in (fields.CPUAllocationPolicy.SHARED, fields.CPUAllocationPolicy.MIXED, None):
        required_cpus = len(instance_cell.cpuset)
        if required_cpus > len(host_cell.cpuset):
            LOG.debug('Not enough host cell CPUs to fit instance cell; required: %(required)d, actual: %(actual)d', {'required': len(instance_cell.cpuset), 'actual': len(host_cell.cpuset)})
            return None
    if instance_cell.cpu_policy in (fields.CPUAllocationPolicy.DEDICATED, fields.CPUAllocationPolicy.MIXED):
        LOG.debug('Instance has requested pinned CPUs')
        required_cpus = len(instance_cell.pcpuset) + cpuset_reserved
        if required_cpus > host_cell.avail_pcpus:
            LOG.debug('Not enough available CPUs to schedule instance. Oversubscription is not possible with pinned instances. Required: %(required)d (%(vcpus)d + %(num_cpu_reserved)d), actual: %(actual)d', {'required': required_cpus, 'vcpus': len(instance_cell.pcpuset), 'actual': host_cell.avail_pcpus, 'num_cpu_reserved': cpuset_reserved})
            return None
        if instance_cell.memory > host_cell.avail_memory:
            LOG.debug('Not enough available memory to schedule instance. Oversubscription is not possible with pinned instances. Required: %(required)s, available: %(available)s, total: %(total)s. ', {'required': instance_cell.memory, 'available': host_cell.avail_memory, 'total': host_cell.memory})
            return None
        instance_cell = _pack_instance_onto_cores(host_cell, instance_cell, num_cpu_reserved=cpuset_reserved)
        if not instance_cell:
            LOG.debug('Failed to map instance cell CPUs to host cell CPUs')
            return None
    if instance_cell.cpu_policy in (fields.CPUAllocationPolicy.SHARED, fields.CPUAllocationPolicy.MIXED, None) and limits:
        LOG.debug('Instance has requested shared CPUs; considering limitations on usable CPU and memory.')
        cpu_usage = host_cell.cpu_usage + len(instance_cell.cpuset)
        cpu_limit = len(host_cell.cpuset) * limits.cpu_allocation_ratio
        if cpu_usage > cpu_limit:
            LOG.debug('Host cell has limitations on usable CPUs. There are not enough free CPUs to schedule this instance. Usage: %(usage)d, limit: %(limit)d', {'usage': cpu_usage, 'limit': cpu_limit})
            return None
        ...
        if ram_usage > ram_limit:
            LOG.debug('Host cell has limitations on usable memory. There is not enough free memory to schedule this instance. Usage: %(usage)d, limit: %(limit)d', {'usage': ram_usage, 'limit': ram_limit})
            return None
    instance_cell.id = host_cell.id
    return instance_cell

@p.trace('_get_flavor_image_meta')
def _get_flavor_image_meta(key: str, flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta', default: ty.Any=None, prefix: str='hw') -> ty.Tuple[ty.Any, ty.Any]:
...
@p.trace('_get_unique_flavor_image_meta')
def _get_unique_flavor_image_meta(key: str, flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta', default: ty.Any=None, prefix: str='hw') -> ty.Any:
    """A variant of '_get_flavor_image_meta' that errors out on conflicts."""
    (flavor_value, image_value) = _get_flavor_image_meta(key, flavor, image_meta, default, prefix)
    if image_value and flavor_value and (image_value != flavor_value):
        msg = _('Flavor %(flavor_name)s has %(prefix)s:%(key)s extra spec explicitly set to %(flavor_val)s, conflicting with image %(image_name)s which has %(prefix)s_%(key)s explicitly set to %(image_val)s.')
        raise exception.FlavorImageConflict(msg % {'prefix': prefix, 'key': key, 'flavor_name': flavor.name, 'flavor_val': flavor_value, 'image_name': image_meta.name, 'image_val': image_value})
    return flavor_value or image_value

@p.trace('get_mem_encryption_constraint')
def get_mem_encryption_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta', machine_type: ty.Optional[str]=None) -> bool:
    ...
    was requested
    """
    (flavor_mem_enc_str, image_mem_enc) = _get_flavor_image_meta('mem_encryption', flavor, image_meta)
    flavor_mem_enc = None
    ...
    if not flavor_mem_enc and (not image_mem_enc):
        return False
    _check_for_mem_encryption_requirement_conflicts(flavor_mem_enc_str, flavor_mem_enc, image_mem_enc, flavor, image_meta)
    requesters = []
    if flavor_mem_enc:
        requesters.append('hw:mem_encryption extra spec in %s flavor' % flavor.name)
    if image_mem_enc:
        requesters.append('hw_mem_encryption property of image %s' % image_meta.name)
...
@p.trace('_check_for_mem_encryption_requirement_conflicts')
def _check_for_mem_encryption_requirement_conflicts(flavor_mem_enc_str, flavor_mem_enc, image_mem_enc, flavor, image_meta):
    if flavor_mem_enc is not None and image_mem_enc is not None and (flavor_mem_enc != image_mem_enc):
        emsg = _('Flavor %(flavor_name)s has hw:mem_encryption extra spec explicitly set to %(flavor_val)s, conflicting with image %(image_name)s which has hw_mem_encryption property explicitly set to %(image_val)s')
        data = {'flavor_name': flavor.name, 'flavor_val': flavor_mem_enc_str, 'image_name': image_meta.name, 'image_val': image_mem_enc}
        raise exception.FlavorImageConflict(emsg % data)

@p.trace('_check_mem_encryption_uses_uefi_image')
def _check_mem_encryption_uses_uefi_image(requesters, image_meta):
    if image_meta.properties.get('hw_firmware_type') == 'uefi':
        return
    emsg = _("Memory encryption requested by %(requesters)s but image %(image_name)s doesn't have 'hw_firmware_type' property set to 'uefi'")
    data = {'requesters': ' and '.join(requesters), 'image_name': image_meta.name}
    raise exception.FlavorImageConflict(emsg % data)

@p.trace('_check_mem_encryption_machine_type')
def _check_mem_encryption_machine_type(image_meta, machine_type=None):
    ...
    if 'q35' not in mach_type:
        raise exception.InvalidMachineType(mtype=mach_type, image_id=image_meta.id, image_name=image_meta.name, reason=_('q35 type is required for SEV to work'))

@p.trace('_get_numa_pagesize_constraint')
def _get_numa_pagesize_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[int]:
    ...
    def check_and_return_pages_size(request):
        if request == 'any':
            ...
            return int(request)
        try:
            return strutils.string_to_bytes(request, return_int=True) / units.Ki
        except ValueError:
            raise exception.MemoryPageSizeInvalid(pagesize=request) from None
    (flavor_request, image_request) = _get_flavor_image_meta('mem_page_size', flavor, image_meta)
    if not flavor_request and image_request:
        raise exception.MemoryPageSizeForbidden(pagesize=image_request, against='<empty>')
    ...
    if image_request and pagesize in (MEMPAGES_ANY, MEMPAGES_LARGE):
        ...
        raise exception.MemoryPageSizeForbidden(pagesize=image_request, against=flavor_request)
    return pagesize

@p.trace('_get_constraint_mappings_from_flavor')
...
@p.trace('_get_numa_cpu_constraint')
def _get_numa_cpu_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[ty.List[ty.Set[int]]]:
    ...
    """
    flavor_cpu_list = _get_constraint_mappings_from_flavor(flavor, 'hw:numa_cpus', parse_cpu_spec)
    ...
    if image_cpu_list is not None:
        raise exception.ImageNUMATopologyForbidden(name='hw_numa_cpus')
    return flavor_cpu_list

@p.trace('_get_numa_mem_constraint')
def _get_numa_mem_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[ty.List[int]]:
    ...
    :return: An ordered list of memory (in GB) to assign to each guest NUMA
        ...
        None.
    """
    flavor_mem_list = _get_constraint_mappings_from_flavor(flavor, 'hw:numa_mem', int)
...
@p.trace('_get_numa_node_count_constraint')
def _get_numa_node_count_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[int]:
    ...
    """
    (flavor_nodes, image_nodes) = _get_flavor_image_meta('numa_nodes', flavor, image_meta)
...
@p.trace('get_cpu_policy_constraint')
def get_cpu_policy_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[str]:
    ...
    """
    (flavor_policy, image_policy) = _get_flavor_image_meta('cpu_policy', flavor, image_meta)
    if flavor_policy and flavor_policy not in fields.CPUAllocationPolicy.ALL:
        raise exception.InvalidCPUAllocationPolicy(source='flavor extra specs', requested=flavor_policy, available=str(fields.CPUAllocationPolicy.ALL))
    if image_policy and image_policy not in fields.CPUAllocationPolicy.ALL:
        raise exception.InvalidCPUAllocationPolicy(source='image properties', requested=image_policy, available=str(fields.CPUAllocationPolicy.ALL))
    if flavor_policy == fields.CPUAllocationPolicy.DEDICATED:
        ...
        if image_policy in (fields.CPUAllocationPolicy.MIXED, fields.CPUAllocationPolicy.DEDICATED):
            raise exception.ImageCPUPinningForbidden()
        cpu_policy = flavor_policy
...
@p.trace('get_cpu_thread_policy_constraint')
def get_cpu_thread_policy_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[str]:
    ...
    if flavor_policy and flavor_policy not in fields.CPUThreadAllocationPolicy.ALL:
        raise exception.InvalidCPUThreadAllocationPolicy(source='flavor extra specs', requested=flavor_policy, available=str(fields.CPUThreadAllocationPolicy.ALL))
    if image_policy and image_policy not in fields.CPUThreadAllocationPolicy.ALL:
        raise exception.InvalidCPUThreadAllocationPolicy(source='image properties', requested=image_policy, available=str(fields.CPUThreadAllocationPolicy.ALL))
...
@p.trace('_get_numa_topology_auto')
def _get_numa_topology_auto(nodes: int, flavor: 'objects.Flavor', vcpus: ty.Set[int], pcpus: ty.Set[int]) -> 'objects.InstanceNUMATopology':
    ...
    for node in range(nodes):
        ...
        cells.append(objects.InstanceNUMACell(id=node, cpuset=cpus & vcpus, pcpuset=cpus & pcpus, memory=mem))
    return objects.InstanceNUMATopology(cells=cells)

@p.trace('_get_numa_topology_manual')
def _get_numa_topology_manual(nodes: int, flavor: 'objects.Flavor', vcpus: ty.Set[int], pcpus: ty.Set[int], cpu_list: ty.List[ty.Set[int]], mem_list: ty.List[int]) -> 'objects.InstanceNUMATopology':
    ...
    for node in range(nodes):
        ...
        for cpu in cpus:
            if cpu > flavor.vcpus - 1:
                raise exception.ImageNUMATopologyCPUOutOfRange(cpunum=cpu, cpumax=flavor.vcpus - 1)
    ...
    if totalmem != flavor.memory_mb:
        raise exception.ImageNUMATopologyMemoryOutOfRange(memsize=totalmem, memtotal=flavor.memory_mb)
    return objects.InstanceNUMATopology(cells=cells)

@p.trace('is_realtime_enabled')
...
@p.trace('_get_vcpu_pcpu_resources')
def _get_vcpu_pcpu_resources(flavor: 'objects.Flavor') -> ty.Tuple[int, int]:
    requested_vcpu = 0
    requested_pcpu = 0
...
@p.trace('_get_hyperthreading_trait')
def _get_hyperthreading_trait(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[str]:
    for (key, val) in flavor.get('extra_specs', {}).items():
        if re.match('trait([1-9][0-9]*)?:%s' % os_traits.HW_CPU_HYPERTHREADING, key):
            return val
    if os_traits.HW_CPU_HYPERTHREADING in image_meta.properties.get('traits_required', []):
        return 'required'
    return None

@p.trace('get_dedicated_cpu_constraint')
def get_dedicated_cpu_constraint(flavor: 'objects.Flavor') -> ty.Optional[ty.Set[int]]:
    ...
    if not pcpus.issubset(cpus):
        msg = _('Mixed instance dedicated vCPU(s) mask is not a subset of vCPUs in the flavor. See "hw:cpu_dedicated_mask"')
        raise exception.InvalidMixedInstanceDedicatedMask(msg)
    return pcpus

@p.trace('get_realtime_cpu_constraint')
def get_realtime_cpu_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[ty.Set[int]]:
    ...
    if not is_realtime_enabled(flavor):
        return None
    (flavor_mask, image_mask) = _get_flavor_image_meta('cpu_realtime_mask', flavor, image_meta)
    ...
    if not vcpus_rt.issubset(vcpus_set):
        msg = _('Realtime policy vCPU(s) mask is configured with RT vCPUs that are not a subset of the vCPUs in the flavor. See hw:cpu_realtime_mask or hw_cpu_realtime_mask')
        raise exception.RealtimeMaskNotFoundOrInvalid(msg)
    return vcpus_rt

@p.trace('get_emulator_thread_policy_constraint')
def get_emulator_thread_policy_constraint(flavor: 'objects.Flavor') -> ty.Optional[str]:
    ...
    """
    emu_threads_policy = flavor.get('extra_specs', {}).get('hw:emulator_threads_policy')
    if not emu_threads_policy:
        return None
    if emu_threads_policy not in fields.CPUEmulatorThreadsPolicy.ALL:
        raise exception.InvalidEmulatorThreadsPolicy(requested=emu_threads_policy, available=str(fields.CPUEmulatorThreadsPolicy.ALL))
    return emu_threads_policy

@p.trace('get_pci_numa_policy_constraint')
def get_pci_numa_policy_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> str:
    ...
    """
    (flavor_policy, image_policy) = _get_flavor_image_meta('pci_numa_affinity_policy', flavor, image_meta)
...
@p.trace('get_vtpm_constraint')
def get_vtpm_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[VTPMConfig]:
    ...
    if version not in fields.TPMVersion.ALL:
        raise exception.Invalid('Invalid TPM version %(version)r. Allowed values: %(valid)s.' % {'version': version, 'valid': ', '.join(fields.TPMVersion.ALL)})
    ...
    if model is None:
        ...
        raise exception.Invalid('Invalid TPM model %(model)r. Allowed values: %(valid)s.' % {'model': model, 'valid': ', '.join(fields.TPMModel.ALL)})
    elif model == fields.TPMModel.CRB and version != fields.TPMVersion.v2_0:
        raise exception.Invalid('TPM model CRB is only valid with TPM version 2.0.')
    return VTPMConfig(version, model)

@p.trace('get_secure_boot_constraint')
def get_secure_boot_constraint(flavor: 'objects.Flavor', image_meta: 'objects.ImageMeta') -> ty.Optional[str]:
    ...
    """
    policy = _get_unique_flavor_image_meta('secure_boot', flavor, image_meta, prefix='os')
    if policy is None:
        return None
    if policy not in fields.SecureBoot.ALL:
        msg = _('Invalid secure boot policy %(policy)r. Allowed values: %(valid)s.')
        raise exception.Invalid(msg % {'policy': policy, 'valid': ', '.join(fields.SecureBoot.ALL)})
    return policy

@p.trace('numa_get_constraints')
def numa_get_constraints(flavor, image_meta):
    ...
    if cpu_policy and (requested_vcpus or requested_pcpus):
        raise exception.InvalidRequest("It is not possible to use the 'resources:VCPU' or 'resources:PCPU' extra specs in combination with the 'hw:cpu_policy' extra spec or 'hw_cpu_policy' image metadata property; use one or the other")
    if requested_vcpus and requested_pcpus:
        raise exception.InvalidRequest("It is not possible to specify both 'resources:VCPU' and 'resources:PCPU' extra specs; use one or the other")
    if requested_pcpus:
        if emu_threads_policy == fields.CPUEmulatorThreadsPolicy.ISOLATE and flavor.vcpus + 1 != requested_pcpus:
            raise exception.InvalidRequest("You have requested 'hw:emulator_threads_policy=isolate' but have not requested sufficient PCPUs to handle this policy; you must allocate exactly flavor.vcpus + 1 PCPUs.")
        if emu_threads_policy != fields.CPUEmulatorThreadsPolicy.ISOLATE and flavor.vcpus != requested_pcpus:
            raise exception.InvalidRequest("There is a mismatch between the number of PCPUs requested via 'resourcesNN:PCPU' and the flavor); you must allocate exactly flavor.vcpus PCPUs")
    ...
    if cpu_thread_policy and hyperthreading_trait:
        raise exception.InvalidRequest("It is not possible to use the 'trait:HW_CPU_HYPERTHREADING' extra spec in combination with the 'hw:cpu_thread_policy' extra spec or 'hw_cpu_thread_policy' image metadata property; use one or the other")
    ...
    if nodes or pagesize or vpmems or (cpu_policy in (fields.CPUAllocationPolicy.DEDICATED, fields.CPUAllocationPolicy.MIXED)):
        cpus = set(range(flavor.vcpus))
        ...
        if cpu_list is None and mem_list is None:
            numa_topology = _get_numa_topology_auto(nodes, flavor, vcpus, pcpus)
            ...
            numa_topology = _get_numa_topology_manual(nodes, flavor, vcpus, pcpus, cpu_list, mem_list)
        else:
            raise exception.ImageNUMATopologyIncomplete()
...
@p.trace('_numa_cells_support_network_metadata')
def _numa_cells_support_network_metadata(host_topology: 'objects.NUMATopology', chosen_host_cells: ty.List['objects.NUMACell'], network_metadata: 'objects.NetworkMetadata') -> bool:
    ...
    if required_physnets:
        for host_cell in chosen_host_cells:
            if 'network_metadata' not in host_cell:
                continue
            required_physnets -= required_physnets.intersection(host_cell.network_metadata.physnets)
        for host_cell in host_topology.cells:
            if 'network_metadata' not in host_cell:
                continue
            if required_physnets.intersection(host_cell.network_metadata.physnets):
                return False
    if required_tunnel:
        for host_cell in chosen_host_cells:
            if 'network_metadata' not in host_cell:
...
@p.trace('numa_fit_instance_to_host')
def numa_fit_instance_to_host(host_topology: 'objects.NUMATopology', instance_topology: 'objects.InstanceNUMATopology', limits: ty.Optional['objects.NUMATopologyLimit']=None, pci_requests: ty.Optional['objects.InstancePCIRequests']=None, pci_stats: ty.Optional[stats.PciDeviceStats]=None):
    ...
    if not (host_topology and instance_topology):
        LOG.debug('Require both a host and instance NUMA topology to fit instance on host.')
        return
    elif len(host_topology) < len(instance_topology):
        LOG.debug('There are not enough NUMA nodes on the system to schedule the instance correctly. Required: %(required)s, actual: %(actual)s', {'required': len(instance_topology), 'actual': len(host_topology)})
        return
    ...
    if not pci_requests and pci_stats:
        host_cells = sorted(host_cells, key=lambda cell: cell.id in [pool['numa_node'] for pool in pci_stats.pools])
    for host_cell_perm in itertools.permutations(host_cells, len(instance_topology)):
        ...
        for (host_cell, instance_cell) in zip(host_cell_perm, instance_topology.cells):
            try:
                cpuset_reserved = 0
                if instance_topology.emulator_threads_isolated and len(chosen_instance_cells) == 0:
                    cpuset_reserved = 1
                got_cell = _numa_fit_instance_cell(host_cell, instance_cell, limits, cpuset_reserved)
        ...
        if len(chosen_instance_cells) != len(host_cell_perm):
            continue
        if pci_requests and pci_stats and (not pci_stats.support_requests(pci_requests, chosen_instance_cells)):
            continue
        if network_metadata and (not _numa_cells_support_network_metadata(host_topology, chosen_host_cells, network_metadata)):
            continue
        return objects.InstanceNUMATopology(cells=chosen_instance_cells, emulator_threads_policy=emulator_threads_policy)

@p.trace('numa_get_reserved_huge_pages')
    ...
    try:
        ...
        for cfg in CONF.reserved_huge_pages:
            try:
                ...
                pagesize = strutils.string_to_bytes(cfg['size'], return_int=True) / units.Ki
            bucket[int(cfg['node'])][pagesize] = int(cfg['count'])
    except (ValueError, TypeError, KeyError):
        raise exception.InvalidReservedMemoryPagesOption(conf=CONF.reserved_huge_pages)
    return bucket

@p.trace('_get_smallest_pagesize')
...
@p.trace('_numa_pagesize_usage_from_cell')
def _numa_pagesize_usage_from_cell(host_cell, instance_cell, sign):
    ...
    for pages in host_cell.mempages:
        if pages.size_kb == pagesize:
            topo.append(objects.NUMAPagesTopology(size_kb=pages.size_kb, total=pages.total, used=max(0, pages.used + instance_cell.memory * units.Ki / pages.size_kb * sign), reserved=pages.reserved if 'reserved' in pages else 0))
        else:
            topo.append(pages)
    return topo

@p.trace('numa_usage_from_instance_numa')
def numa_usage_from_instance_numa(host_topology, instance_topology, free=False):
    ...
    for host_cell in host_topology.cells:
        ...
        new_cell = objects.NUMACell(id=host_cell.id, cpuset=host_cell.cpuset, pcpuset=host_cell.pcpuset, memory=host_cell.memory, cpu_usage=0, memory_usage=0, mempages=host_cell.mempages, pinned_cpus=host_cell.pinned_cpus, siblings=host_cell.siblings)
        ...
        for (cellid, instance_cell) in enumerate(instance_topology.cells):
            if instance_cell.id != host_cell.id:
                continue
            new_cell.mempages = _numa_pagesize_usage_from_cell(new_cell, instance_cell, sign)
            memory_usage = memory_usage + sign * instance_cell.memory
            shared_cpus_usage += sign * len(instance_cell.cpuset)
            if instance_cell.cpu_policy in (None, fields.CPUAllocationPolicy.SHARED):
                continue
