    ...
    def wrapper(ctxt, request_spec):
        ...
        with timer:
            try:
                ...
                if ran:
                    LOG.debug('Request filter %r took %.1f seconds', fn.__name__, timer.elapsed())
        return ran
    return wrapper

@p.trace('isolate_aggregates')
...
def isolate_aggregates(ctxt, request_spec):
    ...
    isolated_aggregates = objects.aggregate.AggregateList.get_non_matching_by_metadata_keys(ctxt, keys, 'trait:', value='required')
    if isolated_aggregates:
        ...
        destination.append_forbidden_aggregates((agg.uuid for agg in isolated_aggregates))
    return True

@p.trace('require_tenant_aggregate')
@trace_request_filter
def require_tenant_aggregate(ctxt, request_spec):
    ...
    if aggregate_uuids_for_tenant:
        ...
        LOG.debug('require_tenant_aggregate request filter added aggregates %s for tenant %r', ','.join(aggregate_uuids_for_tenant), request_spec.project_id)
    elif agg_required:
        LOG.warning('Tenant %(tenant)s has no available aggregates', {'tenant': request_spec.project_id})
        raise exception.RequestFilterFailed(reason=_('No hosts available for tenant'))
    return True

@p.trace('map_az_to_placement_aggregate')
@trace_request_filter
def map_az_to_placement_aggregate(ctxt, request_spec):
    ...
    if not az_hint:
        return False
    aggregates = objects.AggregateList.get_by_metadata(ctxt, key='availability_zone', value=az_hint)
    if aggregates:
        if 'requested_destination' not in request_spec or request_spec.requested_destination is None:
            request_spec.requested_destination = objects.Destination()
        agg_uuids = [agg.uuid for agg in aggregates]
        request_spec.requested_destination.require_aggregates(agg_uuids)
        LOG.debug('map_az_to_placement_aggregate request filter added aggregates %s for az %r', ','.join(agg_uuids), az_hint)
    return True

@p.trace('require_image_type_support')
@trace_request_filter
def require_image_type_support(ctxt, request_spec):
    ...
    if not hasattr(os_traits, trait_name):
        LOG.error('Computed trait name %r is not valid; is os-traits up to date?', trait_name)
        return False
    request_spec.root_required.add(trait_name)
    LOG.debug('require_image_type_support request filter added required trait %s', trait_name)
    return True

@p.trace('transform_image_metadata')
@trace_request_filter
def transform_image_metadata(ctxt, request_spec):
    ...
    if not CONF.scheduler.image_metadata_prefilter:
        return False
    prefix_map = {'hw_cdrom_bus': 'COMPUTE_STORAGE_BUS', 'hw_disk_bus': 'COMPUTE_STORAGE_BUS', 'hw_video_model': 'COMPUTE_GRAPHICS_MODEL', 'hw_vif_model': 'COMPUTE_NET_VIF_MODEL'}
    trait_names = []
    for (key, prefix) in prefix_map.items():
        if key in request_spec.image.properties:
            value = request_spec.image.properties.get(key).replace('-', '_').upper()
    ...
    for trait_name in trait_names:
        LOG.debug('transform_image_metadata request filter added required trait %s', trait_name)
        request_spec.root_required.add(trait_name)
    return True

@p.trace('compute_status_filter')
@trace_request_filter
def compute_status_filter(ctxt, request_spec):
    ...
    LOG.debug('compute_status_filter request filter added forbidden trait %s', trait_name)
    return True

@p.trace('accelerators_filter')
@trace_request_filter
def accelerators_filter(ctxt, request_spec):
    ...
    if request_spec.flavor.extra_specs.get('accel:device_profile'):
        request_spec.root_required.add(trait_name)
        LOG.debug('accelerators_filter request filter added required trait %s', trait_name)
    return True

@p.trace('routed_networks_filter')
@trace_request_filter
def routed_networks_filter(ctxt: nova_context.RequestContext, request_spec: 'objects.RequestSpec') -> bool:
    ...
    for requested_network in requested_networks:
        ...
        if 'port_id' in requested_network and requested_network.port_id:
            port = network_api.show_port(ctxt, requested_network.port_id)['port']
            if port['fixed_ips']:
                subnet_id = port['fixed_ips'][0]['subnet_id']
                try:
                    aggregates = utils.get_aggregates_for_routed_subnet(ctxt, network_api, report_api, subnet_id)
                except exception.InvalidRoutedNetworkConfiguration as e:
                    raise exception.RequestFilterFailed(reason=_('Aggregates not found for the subnet %s') % subnet_id) from e
            else:
                network_id = port['network_id']
        elif 'network_id' in requested_network and requested_network.network_id:
            network_id = requested_network.network_id
        if network_id:
            try:
                aggregates = utils.get_aggregates_for_routed_network(ctxt, network_api, report_api, network_id)
            except exception.InvalidRoutedNetworkConfiguration as e:
                raise exception.RequestFilterFailed(reason=_('Aggregates not found for the network %s') % network_id) from e
        if aggregates:
            LOG.debug('routed_networks_filter request filter added the following aggregates for network ID %s: %s', network_id, ', '.join(aggregates))
            request_spec.requested_destination.require_aggregates(aggregates)
    return True
ALL_REQUEST_FILTERS = [require_tenant_aggregate, map_az_to_placement_aggregate, require_image_type_support, compute_status_filter, isolate_aggregates, transform_image_metadata, accelerators_filter, routed_networks_filter]

@p.trace('process_reqspec')
