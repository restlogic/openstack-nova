...
def _load_auth_plugin(conf):
    auth_plugin = ks_loading.load_auth_from_conf_options(conf, nova.conf.cinder.cinder_group.name)
    if auth_plugin:
        return auth_plugin
    if conf.cinder.auth_type is None:
        LOG.error('The [cinder] section of your nova configuration file must be configured for authentication with the block-storage service endpoint.')
    err_msg = _('Unknown auth type: %s') % conf.cinder.auth_type
    raise cinder_exception.Unauthorized(401, message=err_msg)

@p.trace('_load_session')
def _load_session():
    global _SESSION
    if not _SESSION:
...
def _get_server_version(context, url):
    ...
    return (cinder_api_versions.APIVersion(min_version), cinder_api_versions.APIVersion(current_version))

@p.trace('_get_highest_client_server_version')
def _get_highest_client_server_version(context, url):
    ...
    max_client = cinder_api_versions.APIVersion(cinder_api_versions.MAX_VERSION)
    return min(max_server, max_client)

@p.trace('_check_microversion')
...
@p.trace('_get_cinderclient_parameters')
def _get_cinderclient_parameters(context):
    _load_session()
    auth = _get_auth(context)
    url = None
    (service_type, service_name, interface) = CONF.cinder.catalog_info.split(':')
    service_parameters = {'service_type': service_type, 'interface': interface, 'region_name': CONF.cinder.os_region_name}
    if service_name:
...
@p.trace('is_microversion_supported')
def is_microversion_supported(context, microversion):
    cinderclient(context, microversion=microversion, check_only=True)

@p.trace('cinderclient')
def cinderclient(context, microversion=None, skip_version_check=False, check_only=False):
...
@p.trace('_untranslate_volume_summary_view')
...
@p.trace('_untranslate_volume_type_view')
...
@p.trace('_untranslate_snapshot_summary_view')
...
@p.trace('_translate_attachment_ref')
def _translate_attachment_ref(attachment_ref):
    ...
    if connection_info_data:
        ...
        translated_con_info['driver_volume_type'] = connection_info_data.pop('driver_volume_type', None)
...
@p.trace('translate_cinder_exception')
...
@p.trace('translate_create_exception')
...
@p.trace('translate_volume_exception')
...
@p.trace('translate_attachment_exception')
...
@p.trace('translate_snapshot_exception')
...
@p.trace('translate_mixed_exceptions')
...
class API(object):
    ...
    def get(self, context, volume_id, microversion=None):
        ...
        :raises: CinderAPIVersionNotAvailable if the specified microversion is
            not available.
        """
        item = cinderclient(context, microversion=microversion).volumes.get(volume_id)
    ...
    @translate_volume_exception
    @retrying.retry(stop_max_attempt_number=5, retry_on_exception=lambda e: type(e) == cinder_apiclient.exceptions.InternalServerError)
            ...
            if volume['multiattach']:
                ...
                if instance_uuid:
                    ...
                    if not attachment_id:
                        LOG.warning("attachment_id couldn't be retrieved for volume %(volume_id)s with instance_uuid %(instance_id)s. The volume has the 'multiattach' flag enabled, without the attachment_id Cinder most probably cannot perform the detach.", {'volume_id': volume_id, 'instance_id': instance_uuid})
                else:
                    LOG.warning("attachment_id couldn't be retrieved for volume %(volume_id)s. The volume has the 'multiattach' flag enabled, without the attachment_id Cinder most probably cannot perform the detach.", {'volume_id': volume_id})
    ...
    def initialize_connection(self, context, volume_id, connector):
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error('Initialize connection failed for volume %(vol)s on host %(host)s. Error: %(msg)s Code: %(code)s. Attempting to terminate connection.', {'vol': volume_id, 'host': connector.get('host'), 'msg': str(ex), 'code': ex.code})
                try:
                    self.terminate_connection(context, volume_id, connector)
                except Exception as exc:
                    LOG.error('Connection between volume %(vol)s and host %(host)s might have succeeded, but attempt to terminate connection has failed. Validate the connection and determine if manual cleanup is needed. Error: %(msg)s Code: %(code)s.', {'vol': volume_id, 'host': connector.get('host'), 'msg': str(exc), 'code': exc.code if hasattr(exc, 'code') else None})
    ...
    def create(self, context, size, name, description, snapshot=None, image_id=None, volume_type=None, metadata=None, availability_zone=None):
        ...
        if snapshot is not None:
            ...
            snapshot_id = None
        kwargs = dict(snapshot_id=snapshot_id, volume_type=volume_type, availability_zone=availability_zone, metadata=metadata, imageRef=image_id, name=name, description=description)
    ...
    @translate_volume_exception
    def attachment_create(self, context, volume_id, instance_id, connector=None, mountpoint=None):
        ...
        try:
            attachment_ref = cinderclient(context, '3.44').attachments.create(volume_id, _connector, instance_id)
            return _translate_attachment_ref(attachment_ref)
        except cinder_exception.ClientException as ex:
            with excutils.save_and_reraise_exception():
                if getattr(ex, 'code', None) != 400:
                    LOG.error('Create attachment failed for volume %(volume_id)s. Error: %(msg)s Code: %(code)s', {'volume_id': volume_id, 'msg': str(ex), 'code': getattr(ex, 'code', None)}, instance_uuid=instance_id)
    ...
    def attachment_get(self, context, attachment_id):
        ...
        """
        try:
            attachment_ref = cinderclient(context, '3.44', skip_version_check=True).attachments.show(attachment_id)
            translated_attach_ref = _translate_attachment_ref(attachment_ref.to_dict())
            return translated_attach_ref
        except cinder_exception.ClientException as ex:
            with excutils.save_and_reraise_exception():
                LOG.error('Show attachment failed for attachment %(id)s. Error: %(msg)s Code: %(code)s', {'id': attachment_id, 'msg': str(ex), 'code': getattr(ex, 'code', None)})

    @translate_attachment_exception
    def attachment_update(self, context, attachment_id, connector, mountpoint=None):
        ...
        if mountpoint and 'mountpoint' not in connector:
            ...
            _connector['mountpoint'] = mountpoint
        try:
            attachment_ref = cinderclient(context, '3.44', skip_version_check=True).attachments.update(attachment_id, _connector)
            translated_attach_ref = _translate_attachment_ref(attachment_ref.to_dict())
            return translated_attach_ref
        except cinder_exception.ClientException as ex:
            with excutils.save_and_reraise_exception():
                LOG.error('Update attachment failed for attachment %(id)s. Error: %(msg)s Code: %(code)s', {'id': attachment_id, 'msg': str(ex), 'code': getattr(ex, 'code', None)})

    @translate_attachment_exception
    @retrying.retry(stop_max_attempt_number=5, retry_on_exception=lambda e: type(e) == cinder_apiclient.exceptions.InternalServerError)
    def attachment_delete(self, context, attachment_id):
        try:
            cinderclient(context, '3.44', skip_version_check=True).attachments.delete(attachment_id)
        except cinder_exception.ClientException as ex:
            with excutils.save_and_reraise_exception():
                LOG.error('Delete attachment failed for attachment %(id)s. Error: %(msg)s Code: %(code)s', {'id': attachment_id, 'msg': str(ex), 'code': getattr(ex, 'code', None)})
    ...
    def attachment_complete(self, context, attachment_id):
        ...
        """
        try:
            cinderclient(context, '3.44', skip_version_check=True).attachments.complete(attachment_id)
        except cinder_exception.ClientException as ex:
            with excutils.save_and_reraise_exception():
                LOG.error('Complete attachment failed for attachment %(id)s. Error: %(msg)s Code: %(code)s', {'id': attachment_id, 'msg': str(ex), 'code': getattr(ex, 'code', None)})